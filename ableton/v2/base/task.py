...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\base\task.py
...
class Task(object):
    ...
    def toggle_pause(self):
        if self._state!= KILLED:
            return RUNNING if self._state == PAUSED else self._state
            else:  # inserted
                self._state = PAUSED
        return self
    ...
    def restart(self):
        self.do_restart()
        self._state = RUNNING
        if self._task_manager and self._task_manager.find(self) is None:
...
class TaskGroup(Task):
    ...
    def do_update(self, timer, traceback=None):
        ...
        if self.auto_kill and all_killed:
            ...
            if not self.loop or all_killed:
                    self.restart()
                    return
            else:  # inserted
                return
...
def totask(task):
    if isinstance(task, Task) or not callable(task):
