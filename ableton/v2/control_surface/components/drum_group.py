...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\components\drum_group.py
...
class DrumGroupComponent(PlayableComponent, SlideComponent, Slideable):
    ...
    @property
    def min_pitch(self):
        return self.assigned_drum_pads[0].note if self.assigned_drum_pads else BASE_DRUM_RACK_NOTE

    @property
    def max_pitch(self):
        return self.assigned_drum_pads[-1].note if self.assigned_drum_pads else BASE_DRUM_RACK_NOTE
    ...
    def _color_for_pad(self, pad):
        ...
        if pad == self._selected_drum_pad:
            ...
            if has_soloed_pads and (not pad.solo) and (not pad.mute):
                ...
                if has_soloed_pads and pad.solo:
