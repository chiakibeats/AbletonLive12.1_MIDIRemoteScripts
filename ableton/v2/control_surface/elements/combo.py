...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\elements\combo.py
...
class WrapperElement(CompoundElement, ProxyBase):
    ...
    class ProxiedInterface(CompoundElement.ProxiedInterface):
        ...
        def __getattr__(self, name):
            if PY3 and '_wrapped_control' not in self.outer.__dict__:
...
class ComboElement(WrapperElement):
    ...
    def __init__(self, control=None, modifier=[], *a, **k):
        ...
        self._combo_modifiers = list(map(get_element, modifier)) if not isinstance(modifier, str) and is_iterable(modifier) else [get_element(modifier)]
