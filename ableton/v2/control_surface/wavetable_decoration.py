...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\wavetable_decoration.py
...
class WavetableDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    available_unison_modes = ('None', 'Classic', 'Shimmer', 'Noise', 'Phase Sync', 'Position Spread', 'Random Note')
    ...
    def __init__(self, *a, **k):
        ...
        self._osc_types_provider = NotifyingList(available_values=['1', '2', 'S', 'Mix'], default_value=WavetableOscillatorType.one)
        self._filter_types_provider = NotifyingList(available_values=['1', '2'], default_value=WavetableFilterType.one)
        self._envelope_types_provider = NotifyingList(available_values=['Amp', 'Env2', 'Env3'], default_value=WavetableEnvelopeType.amp)
        self._lfo_types_provider = NotifyingList(available_values=['LFO1', 'LFO2'], default_value=WavetableLfoType.one)
        self._amp_envelope_view_types_provider = NotifyingList(available_values=['Time', 'Slope'], default_value=WavetableEnvelopeViewType.time)
        self._mod_envelope_view_types_provider = NotifyingList(available_values=['Time', 'Slope', 'Value'], default_value=WavetableEnvelopeViewType.time)
    ...
    def _create_parameters(self):
        ...
        return (EnumWrappingParameter(name='Filter', parent=self, values_host=self._filter_types_provider, index_property_host=self._filter_types_provider, values_property='available_values', index_property='index', value_type=WavetableFilterType), EnumWrappingParameter(name='Amp Env View', parent=self, values_host=self._amp_envelope_view_types_provider, index_property_host=self._amp_envelope_view_types_provider, values_property='available_values', index_property='index', value_type=WavetableEnvelopeViewType), EnumWrappingParameter(name='Mod Env View', parent=self, values_host=self, index_property_host=self.MIN_UNISON_VOICE_COUNT, values_property='mono_poly', index_property=lambda index: int(not index), from_index_conversion=lambda index: int(not index)), self.oscillator_switch, self.osc_1_pitch, self.osc_2_pitch, self.envelope_switch, self.lfo_switch, self._osc_1_category_switch, self._osc_1_table_switch, self._osc_2_table_switch, self._osc_1_effect_type_switch, self._osc_2_effect_type_switch)

    def _get_parameter_enabled_state(self, parameter):
        return ParameterState.enabled if parameter.value else ParameterState.disabled
