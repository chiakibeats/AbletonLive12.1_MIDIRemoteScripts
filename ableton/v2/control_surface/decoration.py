...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v2\control_surface\decoration.py
...
class LiveObjectDecorator(CompoundDisconnectable, Proxy):
    ...
    def __init__(self, live_object=None, additional_properties={}):
        ...
        self.__decorated_options = {}
        self.__decorated_parameters = []
    ...
    def _add_non_automatable_enum_parameter(self, name, list, index):
        pass
        self.__decorated_parameters.append(EnumWrappingParameter(name=name, parent=self, values_host=self._live_object, index_property_host=self, values_property=list, index_property=index))
        return self.__decorated_parameters[-1]

    def _add_non_automatable_int_parameter(self, name, property_name, min, max, units):
        pass
        spaced_units = ' ' + units if units != '' else ''
        self.__decorated_parameters.append(IntegerParameter(name=name, parent=self, integer_value_host=self._live_object, integer_value_property_name=property_name, min_value=min, max_value=max, show_as_quantized=False, display_value_conversion=lambda x: str(x) + spaced_units))
        return self.__decorated_parameters[-1]
    ...
    def _add_enum_parameter(self, name, values, default_value):
        ...
        self.__decorated_parameters.append(EnumWrappingParameter(name=name, parent=self, values_host=provider, index_property_host=provider, values_property='available_values', index_property='index', value_type=enum_type))
        return self.__decorated_parameters[-1]

    def _add_switch_option(self, name, pname, labels):
        pass
        self.__decorated_options[pname] = DeviceSwitchOption(name=name, parameter=get_parameter_by_name(self, pname), labels=labels)
        return self.__decorated_options[pname]

    def _add_on_off_option(self, name, pname):
        pass
        self.__decorated_options[pname] = DeviceOnOffOption(name=name, property_host=get_parameter_by_name(self, pname))
        return self.__decorated_options[pname]

    @property
    def parameters(self):
        return tuple(self._live_object.parameters) + tuple(self.__decorated_parameters)

    @property
    def options(self):
        return tuple(self.__decorated_options.values())
...
class PitchParameter(InternalParameter):
    ...
    def _get_linear_value(self):
        return self._decimal_value if self.adjust_finegrain else self._integer_value
    ...
    def automation_state(self):
        ...
        return AutomationState.playing if integer_host_automation_state == AutomationState.playing or decimal_host_automation_state == AutomationState.playing else integer_host_automation_state or decimal_host_automation_state