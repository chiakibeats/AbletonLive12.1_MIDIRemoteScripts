...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v3\control_surface\components\channel_strip.py
...
class ChannelStripComponent(Component, Renderable):
    ...
    mute_button = MappedButtonControl(disabled_color='Mixer.NoTrack', color='Mixer.MuteOff', on_color='Mixer.MuteOn')
    solo_button = ButtonControl(disabled_color='Mixer.NoTrack', color='Mixer.SoloOff', on_color='Mixer.SoloOn')
    arm_button = ButtonControl(disabled_color='Mixer.NoTrack', color='Mixer.ArmOff', on_color='Mixer.ArmOn')
    ...
    def __init__(self, *a, **k):
        ...
        self._track_property_slots = [make_property_slot('solo', self._update_solo_button), make_property_slot('arm', self._update_arm_button), make_property_slot('implicit_arm', self._update_arm_button), make_property_slot('input_routing_type', self._update_arm_button), make_property_slot('color', self._update_track_select_button)]
        self._mixer_device_property_slots = [make_property_slot('crossfade_assign', self._update_crossfade_cycle_button), make_property_slot('sends', self.update)]
    ...
    def solo_button(self, _):
        ...
        for track in chain(self.song.tracks, self.song.return_tracks):
            if track == self._track or (self._track.is_part_of_selection and track.is_part_of_selection):
    ...
    def arm_button(self, _):
        ...
        if action.toggle_arm(self._track, exclusive=arm_exclusive):
            self.notify(self.notifications.Track.arm, cast(str, self._track.name), self._track.arm)
    ...
    def _all_controls(self):
        return chain([self.volume_control, self.pan_control], self.send_controls, self.indexed_send_controls)
