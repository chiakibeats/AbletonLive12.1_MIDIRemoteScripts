...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v3\control_surface\components\drum_group.py
...
class DrumGroupComponent(PlayableComponent, PitchProvider, Renderable):
    ...
    def __getattr__(self, name):
        if name.startswith('set_') and 'scroll' in name:
    ...
    def _on_matrix_pressed(self, button):
        ...
        if not liveobj_valid(self._drum_group_device) or not liveobj_valid(pad):
            return None
        else:  # inserted
    ...
    def _get_selected_drum_pad(self):
        return self._drum_group_device.view.selected_drum_pad if liveobj_valid(self._drum_group_device) else None

    def _any_modifier_pressed(self):
        return any([self.select_button.is_pressed, self.mute_button.is_pressed, self.solo_button.is_pressed, self.delete_button.is_pressed, self.quantize_button.is_pressed])
    ...
    def _update_provided_pitches(self):
        if liveobj_valid(self._selected_drum_pad):
            self.pitches = [self._selected_drum_pad.note]
    ...
    def _update_drum_pad_listeners(self):
        if liveobj_valid(self._drum_group_device):
            ...
            self.__on_color_changed.replace_subjects([pad.chains[0] for pad in visible_drum_pads if pad.chains])
            self._update_assigned_drum_pads()
            self._update_note_translations()
    ...
    def _update_note_translations(self):
        if not self._assigned_drum_pads or not self._can_set_pad_translations():
...
class DrumPadClipboardComponent(ClipboardComponent):
    ...
    def _do_copy(self, obj):
        if not liveobj_valid(obj) or not obj.chains:
    ...
    def _do_paste(self, obj):
        if not liveobj_valid(obj) or self._source_obj.note!= obj.note:
                if obj.chains:
