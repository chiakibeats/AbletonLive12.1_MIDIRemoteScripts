...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v3\control_surface\components\sliced_simpler.py
...
class SlicedSimplerComponent(PlayableComponent, PageComponent, Pageable, PitchProvider, Renderable):
    ...
    def set_simpler_device(self, simpler_device):
        ...
        if liveobj_changed(simpler_device, self._simpler_device):
            ...
            self.__on_file_changed.subject = simpler_device
            self.__on_selected_slice_changed.subject = simpler_device
            self.__on_pad_slicing_changed.subject = simpler_device
            sample = simpler_device.sample if liveobj_valid(simpler_device) else None
            self.__on_slices_changed.subject = sample
            self.__on_slicing_style_changed.subject = sample
            self.__on_track_color_changed.subject = self._target_track.target_track if simpler_device else None
            self.update()
    ...
    def _slices(self):
        return self._simpler_device.sample.slices if self._simpler_setup_is_valid() else []

    def _selected_slice(self):
        return self._simpler_device.view.selected_slice if self._simpler_setup_is_valid() else (-1)
    ...
    def _note_translation_for_button(self, button):
        ...
        return (identifier, self._translation_channel)
    ...
    def __on_file_changed(self):
        return self._simpler_device.sample if liveobj_valid(self._simpler_device) else None
        else:  # inserted
