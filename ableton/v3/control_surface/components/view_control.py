...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ableton\v3\control_surface\components\view_control.py
...
class ViewControlComponent(Component, Renderable):
    ...
    def __init__(self, name='View_Control', session_ring=None, *a, **k):
        ...
        def scroller_factory(scroll_scenes=False, **k):
            ...
            scroller = ViewScroller(self.song, scroll_scenes=scroll_scenes, **k)
            self.register_disconnectable(ObservablePropertyAlias(self, property_host=scroller, property_name='scrolled', alias_name='{}_selection_scrolled'.format(scroller_type.lower())))
            component = ScrollComponent(scroller, parent=self, scroll_skin_name='ViewControl.{}'.format(scroller_type))
            return component
        ...
        self._page_scenes = scroller_factory(scroll_scenes=True, page_size=self._session_ring.num_scenes)
        song = self.song
        view = song.view
