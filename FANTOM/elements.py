...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\FANTOM\elements.py
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_button_matrix([[65] * NUM_TRACKS], 'Arm_Buttons', channels=[track_channels])
        self.add_button_matrix([[66] * NUM_TRACKS], 'Solo_Buttons', channels=[track_channels])
        self.add_button_matrix([[67] * NUM_TRACKS], 'Mute_Buttons', channels=[track_channels])
        self.add_button_matrix([[64] * NUM_SCENES], 'Stop_Track_Buttons', channels=[[i + 1 for i in range(NUM_SCENES)]])
        self.add_button_matrix([range(NUM_SCENES)], 'Scene_Launch_Buttons', msg_type=MIDI_NOTE_TYPE)
        self.add_button_matrix(create_matrix_identifiers(8, 56, width=NUM_TRACKS), 'Clip_Launch_Buttons', msg_type=MIDI_NOTE_TYPE)
        self.add_button_matrix(create_matrix_identifiers(60, 76, width=4), 'Drum_Pads', msg_type=MIDI_NOTE_TYPE, channels=9)
        ...
        self.add_encoder_matrix([[i + 16 for i in range(8)]], 'Device_Controls', is_feedback_enabled=True)
        self.add_encoder_matrix([[72] * NUM_TRACKS], 'Pan_Controls', channels=[track_channels], needs_takeover=False, is_feedback_enabled=True)
        self.add_encoder_matrix([[73] * NUM_TRACKS], 'Volume_Controls', channels=[track_channels], needs_takeover=False, is_feedback_enabled=True)
        self.add_encoder_matrix([[74] * NUM_TRACKS, [75] * NUM_TRACKS], 'Send_Controls', channels=[track_channels, track_channels], needs_takeover=False, is_feedback_enabled=True)
        ...
        self.add_element('Track_Info_Display', TrackInfoDisplayElement, sysex.TRACK_INFO_DISPLAY_HEADER, (sysex.SYSEX_END_BYTE,))
        self.add_element('Scene_Name_Display', SceneNameDisplayElement, sysex.SCENE_NAME_DISPLAY_HEADER, (sysex.SYSEX_END_BYTE,))
