...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\KeyLab\KeyLab.py
...
MESSAGE_DELAY = 0.1
BUTTON_HARDWARE_AND_MESSAGE_IDS = {'session_record_button': (91, 5), 'stop_all_clips_button': (92, 4), 'stop_button': (89, 102), 'play_button': (88, 2), 'record_button': (90, 6), 'loop_button': (93, 55), 'device_left_button': (18, 22), 'device_right_button': (19, 23), 'scene_up_button': (25, 29), 'scene_down_button': (26, 30), 'scene_launch_button': (27, 31)}
ENCODER_CHANNEL = 0
PAD_CHANNEL = 9

def get_button_identifier_by_name(identifier):
    id_pair = BUTTON_HARDWARE_AND_MESSAGE_IDS.get(identifier, None)
    return id_pair[1] if id_pair is not None else id_pair
...
class KeyLab(ArturiaControlSurface):
    ...
    def _create_controls(self):
        self._device_encoders = ButtonMatrixElement(rows=[EncoderElement(MIDI_CC_TYPE, ENCODER_CHANNEL, identifier, Live.MidiMap.MapMode.relative_smooth_binary_offset, name='Device_Encoder_%d_%d' % (col_index, row_index)) for row_index, row in enumerate(row)])
        ...
        self._pads = ButtonMatrixElement(rows=[ButtonElement(True, MIDI_CC_TYPE, PAD_CHANNEL, col_index + row_offset, name='Pad_%d_%d' % (col_index, row_index)) for col_index in enumerate(range(48, 35, -4))])
