...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Komplete_Kontrol_S_Mk3\elements.py
...
def create_kk_encoder(*a, **k):
    return create_encoder(*a, **k, map_mode=MapMode.LinearTwoCompliment, encoder_sensitivity=1.0, mapping_sensitivity=0.1, feedback_delay=-1)
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_encoder(52, 'Arrangement_Position_Encoder')
        self.add_encoder(53, 'Loop_Start_Encoder')
        self.add_encoder(100, 'Track_Volume_Encoder')
        self.add_encoder(101, 'Track_Pan_Encoder')
        self.add_matrix([list(range(80, 88))], 'Volume_Encoders', element_factory=create_kk_encoder, is_feedback_enabled=True)
        self.add_matrix([list(range(88, 96))], 'Pan_Encoders', element_factory=create_kk_encoder, is_feedback_enabled=True)
        for name, identifier in [('Track_Mute_Element', 67), ('Track_Solo_Element', 68)]:
            self.add_element(name, DiscreteValuesElement, identifier, channel=midi.MIDI_CHANNEL, values=range(8))
        ...
        self.add_sysex_element(midi.TRACK_METER_HEADER, 'Track_Meter_Display', send_message_generator=lambda meters: midi.TRACK_METER_HEADER + tuple(meters) + (SYSEX_END,), optimized=True)
        self.add_sysex_element(midi.FOCUS_FOLLOW_HEADER, 'Focus_Follow_Element', send_message_generator=lambda value: midi.FOCUS_FOLLOW_HEADER + value + (SYSEX_END,))

    def add_button(self, *a, **k):
        super().add_button(*a, **k, is_momentary=False)

    def add_encoder(self, identifier, name, **k):
        self.add_element(name, create_kk_encoder, identifier, channel=midi.MIDI_CHANNEL, **k)
