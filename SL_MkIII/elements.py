...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\SL_MkIII\elements.py
...
def create_display_color_element_line(v_position, **k):
    ...
    return (raw_line, ButtonMatrixElement(rows=[raw_line], name='Color_Field_Line_{}'.format(v_position)))
...
def create_selection_field(h_position, v_position):
    ...
    return mixin(CachingControlElement, SysexElement)(sysex_identifier=sysex_identifier, send_message_generator=lambda v: sysex_identifier + (v, sysex.SYSEX_END_BYTE), default_value=0, name='Selection_Field_{}_{}'.format(h_position, v_position))
...
def create_selection_field_line(v_position):
    ...
    return (lines_raw, ButtonMatrixElement(rows=[lines_raw], name='Selection_Field_Line_{}'.format(v_position)))
...
def create_text_display_line(v_position):
    display = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=v_position, width_in_chars=DISPLAY_LINE_WIDTH, num_segments=NUM_DISPLAY_LINE_SEGMENTS, name='Text_Display_{}'.format(v_position))
...
class Elements:
    ...
    def __init__(self, *a, **k):
        ...
        self.shift_button = create_button(91, 'Shift_Button', resource_type=PrioritizedResource)
        ...
        pads_raw = [create_button(offset + col_index, 'Pad_{}_{}'.format(col_index, row_index), msg_type=MIDI_NOTE_TYPE) for col_index in enumerate((96, 112))]
        self.pads = ButtonMatrixElement(rows=pads_raw, name='Pads')
        self.pads_quadratic = ButtonMatrixElement(rows=[pads_raw[0][4:], pads_raw[1][4:], pads_raw[0][:4], pads_raw[1][:4]], name='Pads_Quadratic')
        self.pads_flattened = ButtonMatrixElement(rows=[pads_raw[0] + pads_raw[1]], name='Pads_Flattened')
        self.shifted_pad_row_1 = ButtonMatrixElement(rows=[[with_shift(control) for control in pads_raw[1]]], name='Pad_Row_1_With_Shift')
        ...
        self.slider_leds = ButtonMatrixElement(rows=[[create_rgb_led(index + 54, 'Slider_LED_{}'.format(index)) for index in range(8)]], name='Slider_LEDs')
        self.mixer_soft_button_row_0 = ButtonMatrixElement(rows=[[create_button(identifier, 'Mixer_Soft_Button_{}_0'.format(col_index)) for col_index, identifier in enumerate(range(59, 59 + SESSION_WIDTH))]], name='Mixer_Soft_Button_Row_0')
        self.mixer_soft_button_row_1 = ButtonMatrixElement(rows=[[create_button(identifier, 'Mixer_Soft_Button_{}_1'.format(col_index)) for col_index, identifier in enumerate(range(59 + SESSION_WIDTH, 59 + SESSION_WIDTH * 2))]], name='Mixer_Soft_Button_Row_1')
        self.message_display = SpecialPhysicalDisplayElement(width_in_chars=38, num_segments=NUM_MESSAGE_SEGMENTS, name='Message_Display')
        self.message_display.set_message_parts(sysex.SHOW_MESSAGE_MSG_HEADER, (sysex.SYSEX_END_BYTE,))
        self.center_display_1 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=0, width_in_chars=9, name='Center_Display_1')
        self.center_display_2 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=1, width_in_chars=9, name='Center_Display_2')
        self.mixer_display_1 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=2, width_in_chars=9, name='Mixer_Button_Display_1')
        self.mixer_display_2 = mixin(CachingControlElement, ConfigurablePhysicalDisplayElement)(v_position=3, width_in_chars=9, name='Mixer_Button_Display_2')
        for display in [self.center_display_1, self.center_display_2, self.mixer_display_1, self.mixer_display_2]:
            display.set_message_parts(sysex.SET_PROPERTY_MSG_HEADER, (sysex.SYSEX_END_BYTE,))
        ...
        self.select_buttons = ButtonMatrixElement(rows=[self.select_buttons_raw], name='Select_Buttons')
        self.select_buttons_with_shift_raw = [with_shift(button) for button in self.select_buttons_raw]
        self.display_layout_switch = SysexElement(name='Display_Layout_Switch', send_message_generator=lambda v: sysex.SET_SCREEN_LAYOUT_MESSAGE_HEADER + (v, sysex.SYSEX_END_BYTE), default_value=sysex.EMPTY_SCREEN_LAYOUT_BYTE, optimized=True)
        ...
        self.text_display_lines = [self.text_display_line_0, self.text_display_line_1, self.text_display_line_2, self.text_display_line_3, self.text_display_line_5]
        ...
        self.color_field_lines_0_1_flattened = ButtonMatrixElement(rows=[color_field_line_0_raw + color_field_line_1_raw], name='Color_Field_Lines_0_1_Flattened')
        self.color_field_line_2_with_shift = ButtonMatrixElement(rows=[[with_shift(color_field) for color_field in self.color_field_line_2_raw]], name='Color_Field_Line_2_With_Shift')
        ...
        self.selection_field_lines_0_1_flattened = ButtonMatrixElement(rows=[selection_field_line_0_raw + self.selection_field_line_1_raw], name='Selection_Field_Lines_0_1_Flattened')
        self.selection_field_line_1_with_shift = ButtonMatrixElement(rows=[[with_shift(field) for field in self.selection_field_line_1_raw]], name='Selection_Field_Line_1_With_Shift')
        self.selection_field_line_2_with_shift = ButtonMatrixElement(rows=[[with_shift(field) for field in self.selection_field_line_2_raw]], name='Selection_Field_Line_2_With_Shift')
        ...
        self.encoder_color_fields = ButtonMatrixElement(rows=[[create_display_color_element(index, 1, 'Encoder_Color_Field_{}'.format(index)) for index in range(8)]], name='Encoder_Color_Fields')