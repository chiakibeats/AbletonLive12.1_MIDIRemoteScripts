...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Roland_FA\fa.py
...
class FA(SimpleControlSurface):
    ...
    def _create_controls(self):
        ...
        self._encoders = ButtonMatrixElement(rows=[[make_encoder(index + 70, 'Encoder_%d' % (index,)) for index in range(6)]], name='Encoders')
        ...
        self._pads = ButtonMatrixElement(rows=[make_button(col_index + offset, 'Pad_%d_%d' % (col_index, row_index), msg_type=MIDI_NOTE_TYPE) for col_index in enumerate(range(72, 59, -4))], name='Pads')

    def _create_transport(self):
        self._transport = TransportComponent(name='Transport', is_enabled=False, layer=Layer(play_button=self._play_button, stop_button=self._stop_button, seek_backward_button=self._rwd_button, seek_forward_button=self._ff_button, jump_to_start_button=self._jump_to_start_button))
        self._transport.set_enabled(True)

    def _create_session_recording(self):
        self._session_recording = SessionRecordingComponent(name='Session_Recording', is_enabled=False, layer=Layer(record_button=self._record_button))
        self._session_recording.set_enabled(True)

    def _create_mixer(self):
        self._session_ring = SessionRingComponent(num_tracks=self._encoders.width(), num_scenes=0, is_enabled=False, name='Session_Ring')
        self._mixer = MixerComponent(tracks_provider=self._session_ring, name='Mixer')

    def _create_navigation(self):
        self._navigation = SessionNavigationComponent(session_ring=self._session_ring, name='Navigation', is_enabled=False, layer=Layer(page_left_button=self._s1_button, page_right_button=self._s2_button))
    ...
    def _create_modes(self):
        ...
        self._modes.add_mode('send_a_mode', [LayerMode(self._mixer, Layer(send_controls=self._encoders)), partial(self._set_send_index, 0)])
        self._modes.add_mode('send_b_mode', [LayerMode(self._mixer, Layer(send_controls=self._encoders)), partial(self._set_send_index, 1)])
    ...
    def _create_drums(self):
        self._drums = DrumGroupComponent(name='Drum_Group', is_enabled=False, translation_channel=0, layer=Layer(matrix=self._pads))
