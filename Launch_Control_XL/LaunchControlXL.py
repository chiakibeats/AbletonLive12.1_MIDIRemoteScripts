...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launch_Control_XL\LaunchControlXL.py
...
class LaunchControlXL(IdentifiableControlSurface):
    ...
    def _create_controls(self):
        ...
        self._send_encoders = ButtonMatrixElement(rows=[[make_encoder(13 + i, 'Top_Send_%d' % (i + 1)) for i in range(8)], [make_encoder(29 + i, 'Bottom_Send_%d' % (i + 1)) for i in range(8)]])
        self._pan_device_encoders = ButtonMatrixElement(rows=[[make_encoder(49 + i, 'Pan_Device_%d' % (i + 1)) for i in range(8)]])
        self._volume_faders = ButtonMatrixElement(rows=[[make_slider(77 + i, 'Volume_%d' % (i + 1)) for i in range(8)]])
        ...
        self._select_buttons = ButtonMatrixElement(rows=[make_button_list(chain(range(41, 45), range(57, 61)), 'Track_Select_%d')])
        self._state_buttons = ButtonMatrixElement(rows=[make_button_list(chain(range(73, 77), range(89, 93)), 'Track_State_%d')])
        self._send_encoder_lights = ButtonMatrixElement(rows=[make_button_list([13, 29, 45, 61, 77, 93, 109, 125], 'Top_Send_Encoder_Light_%d'), make_button_list([14, 30, 46, 62, 78, 94, 110, 126], 'Bottom_Send_Encoder_Light_%d')])
        self._pan_device_encoder_lights = ButtonMatrixElement(rows=[make_button_list([15, 31, 47, 63, 79, 95, 111, 127], 'Pan_Device_Encoder_Light_%d')])
        ...
        if midi_bytes[:7] == PREFIX_TEMPLATE_SYSEX:
            if midi_bytes[7] == LIVE_CHANNEL:
                if self._initialize_task.is_running:
                    ...
                    self.update()
            self._c_instance.reset_input_history()
            return
        else:
