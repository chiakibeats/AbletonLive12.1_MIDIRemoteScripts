...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MackieControl_Classic\ChannelStrip.py
            ...
            if sw_id in range(SID_SOLO_BASE, SID_SOLO_BASE + NUM_CHANNEL_STRIPS):
                ...
                if sw_id in range(SID_MUTE_BASE, SID_MUTE_BASE + NUM_CHANNEL_STRIPS):
                    ...
                    if sw_id in range(SID_SELECT_BASE, SID_SELECT_BASE + NUM_CHANNEL_STRIPS):
                        if sw_id - SID_SELECT_BASE is self.__strip_index and value == BUTTON_PRESSED:
                                self.__select_track()
                    else:  # inserted
                        if sw_id in range(SID_VPOD_PUSH_BASE, SID_VPOD_PUSH_BASE + NUM_CHANNEL_STRIPS):
                            if sw_id - SID_VPOD_PUSH_BASE is self.__strip_index and value == BUTTON_PRESSED:
