...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_Pro\Launchpad_Pro.py
    ...
    def __init__(self, *a, **k):
        ...
        for index, val in enumerate(left_button_names):
            if val in ['Session_Record_Button', 'Undo_Button', 'Click_Button']:
                self.add_button(val, 0, (index + 1) * 10, MIDI_CC_TYPE, rec_states if val == 'Session_Record_Button' else default_states=default_states)
                continue
            else:  # inserted
                self.add_modifier_button(val, 0, (index + 1) * 10, MIDI_CC_TYPE, shift_states if val == 'Shift_Button' else default_states=default_states)
                continue
...
class Launchpad_Pro(IdentifiableControlSurface, OptimizedControlSurface):
    ...
    def _create_actions(self):
        self._clip_actions_component = ClipActionsComponent(self._target_track_component, self._actions_component, name='Clip_Actions', is_enabled=False, layer=Layer(duplicate_button=self._midimap['Duplicate_Button'], double_button=self._midimap['Double_Loop_Button'], quantize_button=self._midimap['Quantize_Button']))
    ...
    def _create_session_zooming_modes(self):
        ...
        self._modes.add_mode('session_zooming_mode', [self._session_zooming_manager, session_zooming_button_layer_mode, session_zooming_layer_mode, session_zooming_background_layer_mode])
        self._modes.add_mode('prioritized_session_zooming_mode', [partial(self._layout_switch, consts.SESSION_LAYOUT_SYSEX_BYTE), self._session_zooming_manager, self._prioritized_session_zooming_button_layer_mode, session_zooming_layer_mode, session_zooming_background_layer_mode, self.update])
    ...
    def _create_note_modes(self):
        ...
        self._note_modes.add_mode('chromatic_mode', [partial(self._layout_setup, consts.NOTE_LAYOUT_SYSEX_BYTE), self._clip_delete_layer_mode, note_mode_matrix_translation, scale_setup_mode_button_lighting])
        self._note_modes.add_mode('drum_mode', [partial(self._layout_setup, consts.DRUM_LAYOUT_SYSEX_BYTE), self._setup_drum_group, drum_group_layer_mode, drum_mode_note_matrix_translation])
        ...
        self._modes.add_mode('note_mode', [note_mode_scene_launch_translation, self._note_modes, self._select_note_mode, self._select_target_track, self._clip_actions_component, self._show_playing_clip, self._set_clip_actions_type], behaviour=ReenterBehaviour(self.toggle_detail_view))
    ...
    def _create_track_select_mode(self):
        ...
        self._modes.add_mode('track_select_mode', [partial(self._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), self._session_layer_mode, track_select_layer_mode, self._session_zooming_manager, self._prioritized_session_zooming_button_layer_mode, self._session.update_navigation_buttons], behaviour=SpecialReenterBehaviour('session_mode'))
    ...
    def _create_mute_mode(self):
        ...
        self._modes.add_mode('mute_mode', [partial(self._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), self._session_layer_mode, mute_layer_mode, self._session_zooming_manager, self._prioritized_session_zooming_button_layer_mode, self._session.update_navigation_buttons], behaviour=SpecialReenterBehaviour('session_mode'))
    ...
    def _create_solo_mode(self):
        ...
        self._modes.add_mode('solo_mode', [partial(self._layout_setup, consts.SESSION_LAYOUT_SYSEX_BYTE), self._session_layer_mode, solo_layer_mode, self._session_zooming_manager, self._prioritized_session_zooming_button_layer_mode, self._session.update_navigation_buttons], behaviour=SpecialReenterBehaviour('session_mode'))
    ...
    def _select_note_mode(self):
        ...
        if track is not None and (not track.is_foldable) and (not (track in self.song().return_tracks or track == self.song().master_track or track.is_frozen)):
            pass  # postinserted
        ...
        if len(midi_bytes) < 7:
            ...
            if self._is_challenge_response(midi_bytes) and self._is_response_valid(midi_bytes):
                ...
                if midi_bytes[6] == consts.SYSEX_STATUS_BYTE_LAYOUT and midi_bytes[7] == consts.NOTE_LAYOUT_SYSEX_BYTE[0]:
                    ...
                    if midi_bytes[6] in (consts.SYSEX_STATUS_BYTE_MODE, consts.SYSEX_STATUS_BYTE_LAYOUT):
                        return
                    else:  # inserted
