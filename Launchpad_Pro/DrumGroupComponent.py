...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_Pro\DrumGroupComponent.py
        ...
        if pad == self._selected_drum_pad:
            ...
            if pad.chains:
                if soloed_pads and (not pad.solo):
                    ...
                    if not soloed_pads and pad.mute:
                        button_color = 'DrumGroup.PadMuted'
                    else:  # inserted
                        if soloed_pads and pad.solo:
