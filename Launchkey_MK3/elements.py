...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchkey_MK3\elements.py
...
def create_parameter_display_matrix(command_byte, start_index, name):
    return ButtonMatrixElement(rows=[[create_display_element((command_byte, start_index + index), '{}_Display_{}'.format(name, index)) for index in range(8)]], name='{}_Displays'.format(name))
...
class Elements(LaunchkeyElements):
    ...
    def __init__(self, *a, **k):
        ...
        self.left_button_with_shift = self.with_shift(self.left_button)
        self.right_button_with_shift = self.with_shift(self.right_button)
        self.secondary_up_button = create_button(104, '')
        self.secondary_down_button = create_button(105, '')
        self.device_select_matrix = ButtonMatrixElement(rows=[create_button(offset + col_index, '{}_Device_Select_Button_{}'.format(col_index, row_index), msg_type=MIDI_NOTE_TYPE, channel=0) for col_index in enumerate(range(64, 87, 16))], name='Device_Select_Matrix')
        self.fader_layout_switch = create_button(10, 'Fader_Layout_Switch')
        self.fader_buttons = ButtonMatrixElement(rows=[[create_rgb_button(index + 37, 'Fader_Button_{}'.format(index)) for index in range(SESSION_WIDTH)]], name='Fader_Button')
        self.faders = ButtonMatrixElement(rows=[[create_encoder(index + 53, 'Fader_{}'.format(index)) for index in range(SESSION_WIDTH)]], name='Faders')
        self.master_fader = create_encoder(61, 'Master_Fader')
        self.notification_display = ButtonMatrixElement(rows=[[create_display_element(midi.NOTIFICATION_DISPLAY_COMMAND_BYTES[index], 'Notification_Display_Line_{}'.format(index)) for index in range(2)]], name='Notification_Display')
        ...
        self.master_fader_parameter_name_display = create_display_element((midi.PARAMETER_NAME_DISPLAY_COMMAND_BYTE, midi.MASTER_PARAMETER_DISPLAY_INDEX), 'Master_Fader_Parameter_Name_Display')
        self.master_fader_parameter_value_display = create_display_element((midi.PARAMETER_VALUE_DISPLAY_COMMAND_BYTE, midi.MASTER_PARAMETER_DISPLAY_INDEX), 'Master_Fader_Parameter_Value_Display')
    ...
    def init_display_elements(self, is_88_key_model):
        ...
        for element in vars(self).values():
            if isinstance(element, SimpleDisplayElement):
                ...
                if isinstance(element, ButtonMatrixElement) and isinstance(element[0], SimpleDisplayElement):
