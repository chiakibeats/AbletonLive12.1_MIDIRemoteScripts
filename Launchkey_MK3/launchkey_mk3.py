...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchkey_MK3\launchkey_mk3.py
...
class Launchkey_MK3(InstrumentControlMixin, NovationBase):
    ...
    def _create_session_navigation_layer(self):
        return Layer(up_button='up_button', down_button='down_button', page_left_button='left_button_with_shift', page_right_button='right_button_with_shift')
    ...
    def _create_background(self):
        self._background = BackgroundComponent(name='Background', is_enabled=False, add_nop_listeners=True, layer=Layer(secondary_up_button='secondary_up_button', secondary_down_button='secondary_down_button', device_select_button='device_select_button', unused_matrix=self._elements.device_select_matrix.submatrix[:, 1:], pot_parameter_name_displays='pot_parameter_name_displays', pot_parameter_value_displays='pot_parameter_value_displays', fader_parameter_name_displays='fader_parameter_name_displays', fader_parameter_value_displays='fader_parameter_value_displays'))
        self._background.set_enabled(True)

    def _create_notification(self):
        self._notification_component = NotificationComponent(name='Notifications', is_enabled=False, layer=Layer(display_lines='notification_display'))
        self._notification_component.set_enabled(True)

    def _create_view_control(self):
        self._view_control = NotifyingViewControlComponent(name='Track_Scroller', is_enabled=False, track_provider=self._session_ring, layer=Layer(prev_track_button='left_button', next_track_button='right_button'))
    ...
    def _create_transport(self):
        self._transport = TransportComponent(name='Transport', is_enabled=False, layer=Layer(play_button='play_button', continue_playing_button='play_button_with_shift', alt_stop_button='stop_button', loop_button='loop_button', metronome_button='click_button', capture_midi_button='capture_midi_button'))
    ...
    def _create_undo(self):
        self._undo = UndoRedoComponent(name='Undo', is_enabled=False, layer=Layer(undo_button='undo_button', redo_button='undo_button_with_shift'))
        self._undo.set_enabled(True)

    def _create_quantization(self):
        self._quantization = QuantizationComponent(name='Quantization')
        self._clip_actions = ClipActionsComponent(name='Clip_Actions', quantization_component=self._quantization, is_enabled=False, layer=Layer(quantize_button='quantize_button'))
        self._clip_actions.set_enabled(True)

    def _create_device(self):
        self._device = DeviceComponent(name='Device', is_enabled=False, show_notification=self._notification_component.show_notification, device_bank_registry=self._device_bank_registry, toggle_lock=self.toggle_lock, use_parameter_banks=True, layer=Layer(device_lock_button='device_lock_button'))
        self._device.set_enabled(True)

    def _create_drum_group(self):
        self._drum_group = DrumGroupComponent(name='Drum_Group', is_enabled=False, translation_channel=DRUM_FEEDBACK_CHANNEL, layer=Layer(matrix='drum_pads', scroll_page_up_button='up_button', scroll_page_down_button='down_button'))

    def _create_pot_or_fader_modes(self, modes_type_name):
        modes = ModesComponent(name='{}_Modes'.format(modes_type_name.title()), is_enabled=False, layer=Layer(mode_selection_control='{}_layout_switch'.format(modes_type_name)))
        ...
        def add_pot_or_fader_mixer_mode(parameter_name):
            modes.add_mode(parameter_name, (partial(getattr(self._mixer, 'set_{}_parameter_name'.format(modes_type_name)), parameter_name.replace('_', ' ').title()), AddLayerMode(self._mixer, Layer(**{'{}_controls'.format(parameter_name): elements_name, '{}_parameter_name_displays'.format(modes_type_name): name_displays_element_name, '{}_displays'.format(parameter_name): value_displays_element_name}))))
        modes.add_mode('dummy', None)
        add_pot_or_fader_mixer_mode('volume')
        modes.add_mode('device', AddLayerMode(self._device, Layer(parameter_controls=elements_name, parameter_name_displays=name_displays_element_name, parameter_value_displays=value_displays_element_name)))
    ...
    def _create_fader_button_modes(self):
        self._fader_button_modes = ModesComponent(name='Fader_Modes', is_enabled=False, support_momentary_mode_cycling=False, layer=Layer(cycle_mode_button='fader_button_modes_button'))
        self._fader_button_modes.add_mode('arm', AddLayerMode(self._mixer, Layer(arm_buttons='fader_buttons')), cycle_mode_button_color='DefaultButton.Off')
        self._fader_button_modes.add_mode('track_select', AddLayerMode(self._mixer, Layer(track_select_buttons='fader_buttons')), cycle_mode_button_color='DefaultButton.On')
        self._fader_button_modes.selected_mode = 'arm'
        self._fader_button_modes.set_enabled(True)

    def _create_stop_solo_mute_modes(self):
        self._stop_solo_mute_modes = ModesComponent(name='Stop_Solo_Mute_Modes', is_enabled=False, support_momentary_mode_cycling=False)
        ...
        self._stop_solo_mute_modes.add_mode('stop', AddLayerMode(self._session, Layer(stop_track_clip_buttons=lower_matrix_row)), cycle_mode_button_color='Session.StopClip')
        self._stop_solo_mute_modes.add_mode('solo', AddLayerMode(self._mixer, Layer(solo_buttons=lower_matrix_row)), cycle_mode_button_color='Mixer.SoloOn')
        self._stop_solo_mute_modes.add_mode('mute', AddLayerMode(self._mixer, Layer(mute_buttons=lower_matrix_row)), cycle_mode_button_color='Mixer.MuteOff')
    ...
    def _create_pad_modes(self):
        self._pad_modes = ModesComponent(name='Pad_Modes', is_enabled=False, layer=Layer(mode_selection_control='pad_layout_switch'))
        suppress_scene_launch_buttons = AddLayerMode(self._background, layer=Layer(scene_launch_buttons='scene_launch_buttons'))
        suppress_all_buttons_around_pads = AddLayerMode(self._background, layer=Layer(scene_launch_buttons='scene_launch_buttons', up_button='up_button', down_button='down_button'))
        ...
        self._pad_modes.add_mode('session', LayerMode(self._stop_solo_mute_modes, layer=Layer(cycle_mode_button=self._elements.scene_launch_buttons_raw[1])))
        for i in range(6):
        ...
        self._pad_modes.add_mode('device_select', (suppress_scene_launch_buttons, self._device.show_device_name_and_bank, AddLayerMode(self._device, layer=Layer(bank_select_buttons=upper_matrix_row, prev_button='up_button', next_button='down_button'))))
    ...
    def _select_recording_mode(self):
        if self.application.view.is_view_visible('Session'):
            return 'track' if self._pad_modes.selected_mode == 'drum' else 'listens'
            else:  # inserted
                if 'session':
                    pass  # postinserted
            self._recording_modes.selected_mode = self._recording_modes.selected_mode
        else:  # inserted
