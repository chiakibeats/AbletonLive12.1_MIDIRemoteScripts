...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchkey_Mini_MK3\launchkey_mini_mk3.py
...
from Launchkey_MK3.transport import TransportComponent
from . import midi
...
class Launchkey_Mini_MK3(InstrumentControlMixin, NovationBase):
    ...
    def _create_session_navigation_layer(self):
        return Layer(up_button='scene_launch_button_with_shift', down_button='stop_solo_mute_button_with_shift')
    ...
    def _create_transport(self):
        self._transport = TransportComponent(name='Transport', is_enabled=False, layer=Layer(play_button='play_button', continue_playing_button='play_button_with_shift', capture_midi_button='record_button_with_shift'))
    ...
    def _create_device(self):
        self._device = SimpleDeviceParameterComponent(name='Device', is_enabled=False, layer=Layer(parameter_controls='pots'))

    def _create_drum_group(self):
        self._drum_group = DrumGroupComponent(name='Drum_Group', is_enabled=False, translation_channel=DRUM_FEEDBACK_CHANNEL, layer=Layer(matrix='drum_pads', scroll_page_up_button='scene_launch_button_with_shift', scroll_page_down_button='stop_solo_mute_button_with_shift'))

    def _create_pot_modes(self):
        self._pot_modes = ModesComponent(name='Pot_Modes', is_enabled=False, layer=Layer(mode_selection_control='pot_layout_switch'))
    ...
    def _create_pad_modes(self):
        ...
        bg_mode = AddLayerMode(self._background, layer=Layer(scene_launch_buttons='scene_launch_buttons'))
    ...
    def _select_recording_mode(self):
        if self.application.view.is_view_visible('Session'):
            self.Layer = 'track' if self._pad_modes.selected_mode == 'drum' else 'session'
