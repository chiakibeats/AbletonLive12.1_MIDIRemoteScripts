...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ATOMSQ\colors.py
...
def create_color(red, green, blue, on_value=127, fallback=None):
    color = ComplexColor((ColorPart(red, channel=midi.RED_MIDI_CHANNEL), ColorPart(green, channel=midi.GREEN_MIDI_CHANNEL), ColorPart(blue, channel=midi.BLUE_MIDI_CHANNEL), ColorPart(on_value)))
...
def create_color_for_liveobj(obj, is_scene=False):
    if liveobj_valid(obj) and obj.color_index is not None:
        ...
        return Rgb.GREEN_HALF if is_scene else Rgb.OFF
