...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\ATOMSQ\elements.py
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_button_matrix([[i + 36 for i in range(SESSION_WIDTH)]], 'Lower_Pads', msg_type=MIDI_NOTE_TYPE, is_rgb=True)
        self.add_button_matrix([[i + 52 for i in range(SESSION_WIDTH)]], 'Upper_Pads', msg_type=MIDI_NOTE_TYPE, is_rgb=True)
        self.add_encoder_matrix([[i + 14 for i in range(8)]], 'Encoders', map_mode=MapMode.LinearSignedBit, sensitivity_modifier=self.shift_button)
        ...
        self.add_element('Touch_Strip', TouchStripElement, msg_type=MIDI_PB_TYPE, channel=15, needs_takeover=False, leds=[create_button(led_id, name='Touch_Strip_LED_{}'.format(index)) for index, led_id in enumerate(TOUCH_STRIP_LED_CC_RANGE)])
        ...
        def make_display_line(name, display_id, default_formatting):
            ...
            self.add_sysex_display_line(midi.DISPLAY_HEADER + (display_id, 0, 91, 91, 1), name, partial(generate_display_message, display_id), default_formatting=default_formatting)
