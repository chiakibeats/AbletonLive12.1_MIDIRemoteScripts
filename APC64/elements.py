...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC64\elements.py
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        super().__init__(*a, **k)
        add_button = partial(self.add_button, msg_type=MIDI_NOTE_TYPE, led_channel=midi.FULL_BRIGHTNESS_LED_CHANNEL)
        add_modifier_button = partial(self.add_modifier_button, msg_type=MIDI_NOTE_TYPE, led_channel=midi.FULL_BRIGHTNESS_LED_CHANNEL)
        ...
        add_button_matrix(create_matrix_identifiers(0, 64, width=8, flip_rows=True), 'Pads', channels=midi.FULL_BRIGHTNESS_LED_CHANNEL)
        ...
        self.add_matrix([range(16, 24)], 'Touch_Strips', msg_type=MIDI_PB_TYPE, channels=[range(0, 8)], element_factory=TouchStripElement, sensitivity_modifier=self.shift_button)
        self.add_submatrix(self.touch_strips, 'Touch_Strips_2_thru_7', columns=(2, 8))
        self.add_matrix([range(82, 90)], 'Touch_Elements', msg_type=MIDI_NOTE_TYPE, element_factory=lambda identifier, **k: TouchStripTouchElement(identifier, encoder=self.touch_strips_raw[identifier - 82], **k))
        self.add_element('Firmware_Mode_Element', FirmwareModeElement, sysex_identifier=midi.make_message(midi.MODE_MESSAGE_ID, 0)[:-2], send_message_generator=lambda v: midi.make_message(midi.MODE_MESSAGE_ID, v), use_first_byte_as_value=True)
        ...
        self.add_sysex_element(midi.make_message(midi.SET_DISPLAY_OWNER_ID, 0)[:-2], 'Display_Ownership_Command', lambda v: midi.make_message(midi.SET_DISPLAY_OWNER_ID, v), optimized=True, use_first_byte_as_value=True)

        def generate_display_message(index, text):
            return midi.make_message(midi.DISPLAY_MESSAGE_ID, index, *text + (0,))
        for i in range(3):
            self.add_sysex_display_line(midi.make_message(midi.DISPLAY_MESSAGE_ID, i)[:-1], 'Display_Line_{}'.format(i + 1), partial(generate_display_message, i), default_formatting=Text(max_width=8, justification=Text.Justification.CENTER))