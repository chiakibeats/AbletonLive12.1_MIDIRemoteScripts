...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\APC64\render_to_clip.py
...
def get_notes_to_render(note_ons, note_offs, clip_length):
    ...
    for note_on in note_ons:
        for i, note_off in enumerate(note_offs):
            if note_off.pitch == note_on.pitch and note_off.position > note_on.position:
                if note_off.position <= clip_length:
                    notes_to_render.append(MidiNoteSpecification(pitch=note_on.pitch, start_time=note_on.position, duration=note_off.position - note_on.position, velocity=note_on.velocity, mute=False))
                else:
...
def wrap_note(note_on, note_off, clip_length):
    ...
    return [MidiNoteSpecification(pitch=note_on.pitch, start_time=-WRAPPED_NOTE_OFFSET, duration=start_duration + WRAPPED_NOTE_OFFSET, velocity=note_on.velocity, mute=False), MidiNoteSpecification(pitch=note_on.pitch, start_time=note_on.position, duration=end_duration + WRAPPED_NOTE_OFFSET, velocity=note_on.velocity, mute=False)]
...
def get_firmware_note_specification(data_bytes):
    ...
    return FirmwareNoteSpecification(pitch=data_bytes[PITCH_INDEX], velocity=data_bytes[VELOCITY_INDEX], position=to_absolute_beat_time(position_and_type - event_type), is_note_on=bool(event_type))
