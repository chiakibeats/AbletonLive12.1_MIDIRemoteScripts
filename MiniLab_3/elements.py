...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\MiniLab_3\elements.py
...
def create_rgb_pad(identifier, name, **k):
    return create_sysex_sending_button(identifier, name, LED_HEADER + (PAD_ID_TO_SYSEX_ID[identifier],), is_rgb=True, **k)
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_encoder(29, 'Shifted_Display_Encoder', map_mode=MapMode.LinearBinaryOffset)
        self.add_button(118, 'Display_Encoder_Button')
        self.add_button(119, 'Shifted_Display_Encoder_Button')
        self.add_matrix([[86, 87, 89, 90, 110, 111, 116, 117]], 'Encoders', element_factory=RealigningEncoderElement)
        ...
        self.add_matrix([range(36, 44)], 'Pad_Bank_A', msg_type=MIDI_NOTE_TYPE, channels=9, element_factory=create_rgb_pad)
        self.add_matrix([range(44, 52)], 'Pad_Bank_B', msg_type=MIDI_NOTE_TYPE, channels=9, element_factory=create_rgb_pad)
        ...
        self.add_element('Firmware_Element', SysexElement, sysex_identifier=SYSEX_HEADER + (2, 0, 64), is_private=True)