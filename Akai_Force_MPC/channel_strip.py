...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Akai_Force_MPC\channel_strip.py
...
class ChannelStripComponent(ChannelStripComponentBase):
    ...
    def mpc_mute_button(self, _):
        track = self.track
        if not liveobj_valid(track) or track != self.song.master_track:
    ...
    def _track_has_visible_crossfade_assignment_buttons(self):
        track = self.track
        return liveobj_valid(track) and track != self.song.master_track and (track.has_audio_output or False)
