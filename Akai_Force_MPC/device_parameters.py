...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Akai_Force_MPC\device_parameters.py
...
def display_style_for_parameter(parameter):
    ...
    display_style = DISPLAY_STYLE_BIIPOLAR if is_parameter_bipolar(parameter) else DISPLAY_STYLE_UNIPOLAR if liveobj_valid(parameter) else parameter
...
class DeviceParameterComponent(DeviceParameterComponentBase):
    ...
    def device_on_off_parameter(self):
        ...
        return device.parameters[0] if liveobj_valid(device) else None
    ...
    def _toggle_device_enabled_status(self):
        ...
        if not liveobj_valid(on_off) or on_off.is_enabled:
