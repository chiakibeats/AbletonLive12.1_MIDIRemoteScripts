...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\session_component.py
...
class ClipSlotComponent(SpecialClipSlotComponent):
    ...
    def _feedback_value(self, track, slot_or_clip):
        ...
        if slot_or_clip.is_triggered and (not slot_or_clip.will_record_on_start):
            return Blink(color1=CLIP_PLAYING_COLOR, color2=IndexedColor(clip_color), speed=TRIGGERED_CLIP_BLINK_SPEED) if isinstance(slot_or_clip, Live.Clip.Clip) else 'Session.EmptySlotTriggeredPlay'
...
class SceneComponent(SpecialSceneComponent):
    ...
    def build_clip_slot_list(self):
        ...
        def slot_for_track(mixable):
            if old_hasattr(mixable, 'clip_slots'):
                pass
            if len(mixable.clip_slots) == 0:
                return None
            else:
