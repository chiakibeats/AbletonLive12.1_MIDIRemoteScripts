...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\device_navigation.py
...
class RackBank2Device(EventObject):
    ...
    def rack_device(self):
        return self._rack_device if liveobj_valid(self._rack_device) else None
    ...
    @listenable_property
    def is_active(self):
        return self._rack_device.is_active if liveobj_valid(self._rack_device) else False
    ...
    @property
    def _live_ptr(self):
        return self._rack_device._live_ptr + 1 if liveobj_valid(self._rack_device) else 0
...
def collect_devices(component, track_or_chain, nesting_level=0):
    ...
    for device in chain_devices:
        ...
        devices.extend(collect_devices(component, nested_device_parent(device), nesting_level=nesting_level + 1))
        continue
    return devices
...
class DeviceNavigationComponent(DeviceNavigationComponentBase):
    ...
    def __init__(self, device_bank_registry=None, banking_info=None, delete_handler=None, track_list_component=None, *a, **k):
        ...
        self._modes.add_mode('default', [partial(self.chain_selection.set_parent, None), partial(self.bank_selection.set_device, None)])
    ...
    def _on_select_button_released_immediately(self, button):
        if not self._in_device_enabling_mode():
            ...
            if self._delete_handler and self._delete_handler.is_deleting:
                ...
                if self.selected_object == device_or_pad and device_or_pad!= self._selected_on_previous_press:
    ...
    def _toggle_device(self, device):
        if not liveobj_valid(device) or device.parameters[0].is_enabled:
    ...
    def __on_items_changed(self):
        ...
        if lost_selection and isinstance(selected_item, RackBank2Device):
            for item in new_items:
                if isinstance(item, RackBank2Device) and item.rack_device == selected_item.rack_device:
                    ...
                    continue
        lost_selection_on_empty_pad = new_items and is_drum_pad(new_items[(-1)]) and (lost_selection or False)
    ...
    def _begin_move_device(self, device):
        if self.move_device.is_enabled() or device.type!= Live.Device.DeviceType.instrument:
    ...
    def _on_reselecting_object(self, device):
        if not liveobj_valid(device) or not device.can_have_chains or (not device.can_have_drum_pads):
                self._toggle(device)
                return
            else:  # inserted
                return None
    ...
    def _(self, bank_2_device):
        ...
        if liveobj_valid(device) and device.can_have_chains and (not device.can_have_drum_pads):
                    self._toggle(device)
    ...
    def _update_device(self):
        if self._should_select_drum_pad() or not self._is_drum_rack_selected():
                self._modes.selected_mode = 'default'
