...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\mute_solo_stop.py
...
class MuteSoloStopClipComponent(Component, Messenger):
    MESSAGE_FOR_MODE = {'mute': 'Mute: %s', 'solo': 'Solo: %s', 'stop': 'Stop Clips: %s'}
    ...
    def __init__(self, item_provider=None, solo_track_button=None, mute_track_button=None, stop_clips_button=None, track_list_component=None, cancellation_action_performers=[], *a, **k):
        ...
        self._solo_button_handler = GlobalMixerActionComponent(parent=self, track_list_component=track_list_component, mode='solo', mode_active_color='Mixer.SoloOn', mode_locked_color='Mixer.LockedSoloMode', default_color_indicator=TrackStateColorIndicator(item_provider=item_provider, track_property='solo', property_active_color='Mixer.SoloOn', song=self.song), immediate_action=lambda: toggle_mixable_solo(item_provider.selected_item, self.song))
        self._solo_button_handler.layer = Layer(action_button=solo_track_button)
        self._mute_button_handler = GlobalMixerActionComponent(parent=self, track_list_component=track_list_component, mode='mute', mode_active_color='Mixer.MuteOff', mode_locked_color='Mixer.LockedMuteMode', default_color_indicator=TrackStateColorIndicator(item_provider=item_provider, track_property='mute', property_active_color='Mixer.MuteOff', song=self.song), immediate_action=lambda: toggle_mixable_mute(item_provider.selected_item, self.song))
        self._mute_button_handler.layer = Layer(action_button=mute_track_button)
        self._stop_button_handler = GlobalMixerActionComponent(parent=self, track_list_component=track_list_component, mode='stop', immediate_action=partial(stop_clip_in_selected_track, self.song), mode_locked_color='StopClips.LockedStopMode')
    ...
    def __on_mode_locked_changed(self, is_locked, button_handler):
        if is_locked:
            ...
            self._currently_locked_button_handler = None
        return self._currently_locked_button_handler.mode if is_locked else None
        else:  # inserted
            self._track_list.locked_mode = None
