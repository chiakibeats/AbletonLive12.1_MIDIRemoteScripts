...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\clip_control.py
...
class LoopSettingsControllerComponent(LoopSettingsControllerComponentBase):
    ...
    def timeline_navigation(self):
        return getattr(self.clip, 'timeline_navigation', None) if liveobj_valid(self.clip) else None
...
class AudioClipSettingsControllerComponent(AudioClipSettingsControllerComponentBase):
    ...
    def __init__(self, *a, **k):
        ...
        self._audio_clip_parameters = [WarpSetting(name=PARAMETERS_AUDIO[0], parent=self._audio_clip_model, source_property='warp_mode'), PitchSetting(name=PARAMETERS_AUDIO[1], parent=self._audio_clip_model, source_property='pitch_coarse', min_value=(-49.0), max_value=49.0, unit='st'), PitchSetting(name=PARAMETERS_AUDIO[2], parent=self._audio_clip_model, source_property='pitch_fine', min_value=(-51.0), max_value=51.0, unit='ct'), GainSetting(name=PARAMETERS_AUDIO[3], parent=self._audio_clip_model, source_property='gain')]
        self._playhead_real_time_data = RealTimeDataComponent(channel_type='playhead', parent=self)
        self._waveform_real_time_data = RealTimeDataComponent(channel_type='waveform', parent=self)
...
def register_matrix_mode(matrix_map, name, modes_component=None, parent_path=None):
    def find_leaf(tree, path):
        ...
        return sub_tree if len(rest) == 0 else find_leaf(sub_tree, rest)
...
class MatrixModeWatcherComponent(Component):
        ...
        def connect_listeners(dct):
            for key, value in dct.items():
                if key == 'modes_component':
                    self.register_slot(value, self.__on_matrix_mode_changed, 'selected_mode')
                    continue
                else:  # inserted
    ...
    def __on_matrix_mode_changed(self, mode):
        if mode is not None:
            ...
            return None
_MATRIX_MODE_PATH_TO_DATA = {'matrix_modes.note.instrument.play': {'Fold': True, 'NumDisplayKeys': 0, 'ShowGridWindow': False, 'ShowScrollbarCursor': False, 'NumPitches': 128, 'PitchOffset': 0, 'ShowStepLengthGrid': False, 'ShowMultipleGridWindows': False}, 'matrix_modes.note.instrument.sequence': {'Fold': False, 'NumDisplayKeys': 23, 'ShowGridWindow': True, 'ShowScrollbarCursor': True, 'NumPitches': 128, 'PitchOffset': 0, 'ShowStepLengthGrid': True, 'ShowMultipleGridWindows': False}, 'matrix_modes.note.instrument.split_melodic_sequencer': {'Fold': True, 'NumDisplayKeys': 0, 'ShowGridWindow': False, 'ShowScrollbarCursor': False, 'NumPitches': 128, 'PitchOffset': 0, 'ShowStepLengthGrid': True, 'ShowMultipleGridWindows': False}, 'matrix_modes.note.drums.64pads': {'Fold': False, 'NumDisplayKeys': 17, 'ShowGridWindow': True, 'ShowScrollbarCursor': True, 'NumPitches
_DEFAULT_VIEW_DATA = {'Fold': True, 'NumDisplayKeys': 0, 'ShowGridWindow': False, 'ShowScrollbarCursor': False, 'MinPitch': DEFAULT_START_NOTE, 'MaxSequenceablePitch': DEFAULT_START_NOTE, 'MinSequenceablePitch': DEFAULT_START_NOTE, 'PageIndex': 0, 'PageLength': 1.0, 'MinGridWindowPitch': DEFAULT_START_NOTE, 'MaxGridWindowPitch': DEFAULT_START_NOTE, 'NumPitches': 128, 'PitchOffset': 0, 'ShowStepLengthGrid': False, 'IsRecording': False, 'ShowMultipleGridWindows': False}
...
class MidiClipControllerComponent(Component):
    ...
    def external_regions_of_interest_creator(self, region_of_interest_creator):
        ...
        return {'grid_window': region_of_interest_creator(start_identifier=self.grid_window_focus, getter=lambda: (grid_start_time(), grid_start_time() + self._most_recent_page_length))}
    ...
    def _focus_grid_window(self):
        if not liveobj_valid(self.clip) or self.get_static_view_data()['ShowGridWindow']:
    ...
    def matrix_mode_path(self):
        return self._matrix_mode_watcher.matrix_mode_path if self._matrix_mode_watcher is not None else None
    ...
    def _update_minimum_sequenceable_pitch(self):
        return self._most_recent_editable_pitches[0] if self.matrix_mode_path() == 'matrix_modes.note.instrument.sequence' else self._most_recent_base_note

    def _update_note_colors(self):
        matrix_mode = self.matrix_mode_path()
        return matrix_mode is not None and matrix_mode.startswith('matrix_modes.note.drums') or None
            in_correct_mode = matrix_mode == 'matrix_modes.session'
    ...
    def _configure_visualisation(self):
        ...
        if liveobj_valid(visualisation) and liveobj_valid(self.clip) and self._real_time_data_attached:
                    ...
                    return {'ClipColor': color.as_remote_script_color(), 'PageIndex': self._most_recent_page_index, 'PageLength': float(self._most_recent_page_length), 'RowStartTimes': make_vector(self._most_recent_row_start_times) and (not self.clip.is_overdubbing), 'MinPitch': make_vector([float(end) for pitch, (start, end) in self._most_recent_editing_note_regions]), 'NoteSettingsMode': float(visible_region.start), '_note_settings_component': float(visible_region.end), 'is_enabled': float(visible_region.is_enabled), 'NoteSettingsTouched': float(visible_region._note_editor_settings_component), 'is_touched': float(visible_region.is_touched), 'EditingNotePitches': float(visible_region..0), 'pitch': float(visible_region.pitch), 'start': float(visible_region.start), 'end': float(visible_region.end), 'end': float(visible_region.end), 'end': float(visible_region.end), 'end': float(visible_region.end), 'end':
                        pass  # postinserted
