...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\automation.py
...
class AutomationComponent(AutomationComponentBase):
    ...
    def _on_parameter_provider_changed(self, provider):
        self.notify_device()
        provider = provider if getattr(self.parameter_provider, 'device', None) is not None else None
        else:  # inserted
            self.__on_device_changed.subject = None
