...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\device_decoration.py
...
class SimplerPositions(EventObject):
    ...
    def _convert_sample_time(self, sample_time):
        pass
        sample = self._simpler.sample
        return sample.sample_to_beat_time(sample_time) if liveobj_valid(sample) and sample.warping else None
        else:  # inserted
            return sample_time
...
class PedalDeviceDecorator(LiveObjectDecorator):
    def __init__(self, *a, **k):
        ...
        self.mid_freq_option = DeviceSwitchOption(name='Mid Freq', parameter=get_parameter_by_name(self, 'Mid Freq'))
        self.register_disconnectables(self.options)
...
class DrumBussDeviceDecorator(LiveObjectDecorator):
    def __init__(self, *a, **k):
        ...
        self.compressor_option = DeviceOnOffOption(name='Compressor', property_host=get_parameter_by_name(self, 'Compressor On'))
        self.register_disconnectables(self.options)
