...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\shifter.py
...
class ShifterDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        self._pitch_mode_parameter = EnumWrappingParameter(name='Pitch Mode', parent=self, values_host=self._live_object, index_property_host=self, values_property='pitch_mode_list', index_property='pitch_mode_index')
        self._pitch_bend_range_parameter = IntegerParameter(name='Pitch Bend Range', parent=self, integer_value_host=self._live_object, integer_value_property_name='pitch_bend_range', min_value=self.MIN_PITCH_BEND_RANGE, max_value=self.MAX_PITCH_BEND_RANGE, show_as_quantized=False, display_value_conversion=lambda x: unicode(x) + ' st')
        self._additional_parameters = (self._pitch_mode_parameter, self._pitch_bend_range_parameter)
        self.on_pitch_mode_change.subject = self._pitch_mode_parameter
        self.on_pitch_mode_change()
        self._options = tuple([DeviceOnOffOption(name=n, property_host=get_parameter_by_name(self, p)) for n, p in [('Delay', 'Delay On'), ('Delay Sync', 'Delay Sync'), ('Lfo Sync', 'Lfo Sync'), ('Lfo Spin', 'Lfo Spin'), ('Wide', 'Wide'), ('RM Drive', 'RM Drive'), ('Env. Follow', 'Env On')]])
...
class ShifterDeviceComponent(DeviceComponentWithTrackColorViewData):
    ...
    _PRIMARY_LFO_PARAMETERS = frozenset(('Amount', 'Duty Cycle', 'Offset', 'Rate', 'S. Rate', 'Waveform'))
    _SECONDARY_LFO_PARAMETERS = frozenset(('Amount', 'Duty Cycle', 'Offset', 'Phase', 'Rate', 'S. Rate', 'S&H Width', 'Spin', 'Waveform'))
    ...
    def _adjustment_view_data(self):
        ...
        touched_parameters = [self.parameters[button.index] for button in self.parameter_touch_buttons if not button.is_pressed or self.parameters[button.index].parameter.state == ParameterState.enabled]
        ...
        return {'AdjustingLfo': adjusting_lfo, 'AdjustingSecondaryLfo': adjusting_secondary_lfo}
    ...
    def _configuration_view_data(self):
        ...
        return {'LfoLeft': lfo_left, 'LfoRight': lfo_right}
