...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\operator.py
...
class OperatorDeviceDecorator(EventObject, LiveObjectDecorator):
    ...
    def __init__(self, *a, **k):
        ...
        self._osc_types_provider = NotifyingList(available_values=['A', 'B', 'C', 'D'], default_value=OscillatorType.a)
        self._env_feature_provider = NotifyingList(available_values=['Time', 'Level'], default_value=EnvelopeFeature.time)
        self._sloped_env_feature_provider = NotifyingList(available_values=['Time', 'Slope', 'Level'], default_value=SlopedEnvelopeFeature.time)
...
class OperatorDeviceComponent(DeviceComponentWithTrackColorViewData):
    FILTER_PARAMETER_NAMES = ['Filter Type', 'Filter Freq', 'Filter Res']
    FILTER_BANK = 2
    LARGE_PARAMETERS_LIST = [False] * 8
    ENVELOPE_PREFIXES = ['Ae', 'Be', 'Ce', 'De', 'Fe', 'Le', 'Pe']

    def __init__(self, *a, **k):
        super(OperatorDeviceComponent, self).__init__(*a, **k)
        self._bank_configuration = {1: BankConfiguration(lambda: 'Operator%i' % self.selected_oscillator, ButtonRange(3, 6)), self.FILTER_BANK: BankConfiguration(const('Filter'), ButtonRange(1, 3)), 3: BankConfiguration(const('Filter Env.'), ButtonRange(2, 5)), 6: BankConfiguration(const('LFO'), ButtonRange(2, 5)), 8: BankConfiguration(const('Pitch'), ButtonRange(2, 5))}
    ...
    def _envelope_visualisation_data(self):
        ...
        return {'EnvelopeName': config.name_function(), 'EnvelopeLeft': VisualisationGuides.light_left_x(config.button_range.left_index), 'EnvelopeRight': VisualisationGuides.light_right_x(config.button_range.right_index), 'EnvelopeShow': make_vector(list(shown_features)), 'EnvelopeFocus': make_vector(list(focused_features)), 'FilterVisible': self._should_show_filter_visualisation(), 'FilterLeft': VisualisationGuides.light_left_x(1), 'FilterRight': VisualisationGuides.light_right_x(3), 'FilterFocus': self._is_filter_parameter_touched()}
