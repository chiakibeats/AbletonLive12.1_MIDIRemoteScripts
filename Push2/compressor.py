...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\compressor.py
...
class CompressorDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        for event, func in [('input_routing_type', self.notify_input_routing_type_index), ('input_routing_channel', self.notify_input_routing_channel_index)]:
        ...
        self._options = tuple([self.register_disconnectable(make_option(option_name, param_name)) for option_name, param_name in [('Auto Release', 'Auto Release On/Off'), ('Makeup', 'Makeup'), ('Listen', 'S/C Listen'), ('Sidechain', 'S/C On')]])
        ...
        self._additional_parameters = self.register_disconnectables((make_parameter('Input Type', 'available_input_routing_types', 'input_routing_type_index'), make_parameter('Input Channel', 'available_input_routing_channels', 'input_routing_channel_index'), self._position_parameter))
    ...
    def input_channel_position_index(self):
        return self._input_router.input_channel_position_index if self._input_router.has_input_channel_position else 0
...
class CompressorInputRouterMixin(object):
    ...
    def _get_routing_host(self):
        return self._compressor if liveobj_valid(self._compressor) else self._null_routing_host
    ...
    def _register_listeners(self):
        self._registered_listeners = [self.register_slot(subject=self._get_routing_host(), event_name='available_%ss' % self._current_target_property, listener=self.notify_routing_targets), self.register_slot(subject=self._get_routing_host(), event_name=self._current_target_property, listener=self.__on_target_changed)]
