...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\routing.py
...
class Router(EventObject):
    ...
    def _register_listeners(self):
        self.register_slot(MultiSlot(subject=self._song.view, event_name_list=('selected_track', self._current_target_property), listener=self.__on_current_routing_changed))
        self.register_slot(MultiSlot(subject=self._song.view, event_name_list=('selected_track', 'available_%ss' % self._current_target_property), listener=self.__on_routings_changed))
...
def can_combine_targets(targets, postfixes):
    if len(targets) == len(postfixes) and all(map(_target_has_postfix, zip(targets, postfixes))):
...
class InputChannelAndPositionRouter(EventObject):
    ...
    def input_type_name(self):
        ...
        return getattr(current_target.attached_object, 'name', '') if current_target else ''
...
class RoutingChannel(RoutingTarget):
    ...
    def __init__(self, realtime_channel=None, *a, **k):
        ...
        self._layout_names = {Live.Track.RoutingChannelLayout.mono: 'mono', Live.Track.RoutingChannelLayout.midi: 'midi', Live.Track.RoutingChannelLayout.stereo: 'stereo'}
...
class RoutingTypeList(RoutingTargetList):
    ...
    def selected_track(self):
        if self.selected_index < 0:
            ...
            return attached_object if isinstance(attached_object, Live.Track.Track) else None
    ...
    def _make_targets(self):
        ...
        if self._router.has_input_channel_position:
            ...
            def stripped_name(target_name):
                return target_name[:-strip_length] if len(target_name) > strip_length else self._router.input_type_name
...
class RoutingControlComponent(ModesComponent):
    ...
    def monitoring_state_index(self):
        return self.song.view.selected_track.current_monitoring_state if self.can_monitor else None
