...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\hybrid_reverb.py
...
class HybridReverbDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        self.main_section_labels = ['Convolution', 'Algo', 'Mix', 'EQ']
        ...
        self.band = EnumWrappingParameter(name='Band', parent=self, values_host=self._band_names_provider, index_property_host=self._band_names_provider, values_property='available_values', index_property='index')
        self._main_section_provider = NotifyingList(available_values=self.main_section_labels, default_value=0)
        self.main_section = EnumWrappingParameter(name='Section', parent=self, values_host=self._main_section_provider, index_property_host=self._main_section_provider, values_property='available_values', index_property='index')
        self.feedback_ms_sync_switch = DeviceSwitchOption(name='Ms Sync Switch', parameter=self.pre_delay_sync_live_parameter, labels=self.pre_delay_sync_labels)
        self.eq_low_type_switch = DeviceSwitchOption(name='Low Type Switch', parameter=self.eq_low_type_live_parameter, labels=self.filter_types_labels)
        self.eq_high_type_switch = DeviceSwitchOption(name='High Type Switch', parameter=self.eq_high_type_live_parameter, labels=self.filter_types_labels)
        self.ir_post_processing_bool = BoolWrappingParameter(name='Ir Post Processing Bool', parent=self, property_host=self, source_property='ir_time_shaping_on')
        self.ir_post_processing_option = DeviceOnOffOption(name='Shape', property_host=self.ir_post_processing_bool)
        self.eq_on_option = DeviceOnOffOption(name='EQ', property_host=self.eq_on_live_parameter)
        self.freeze_option = DeviceOnOffOption(name='Freeze', property_host=self.freeze_live_parameter)
        self.freeze_in_option = DeviceOnOffOption(name='Freeze In', property_host=self.freeze_in_live_parameter)
        self.prealgo_option = DeviceOnOffOption(name='Pre Algo', property_host=self.eq_prealgo_live_parameter)
        self.bass_mono_option = DeviceOnOffOption(name='Bass Mono', property_host=self.bass_mono_live_parameter)
        self.ir_attack_time_parameter = WrappingParameter(name='Ir Attack Time', parent=self, property_host=self, source_property='ir_attack_time', display_value_conversion=to_ms_display, from_property_value=user_to_linear_exp(0.0, 3.0, 3.33), to_property_value=linear_to_user_exp(0.0, 3.0, 3.33))
        self.ir_decay_time_parameter = WrappingParameter(name='Ir Decay Time', parent=self, property_host=self, source_property='ir_decay_time', display_value_conversion=to_ms_display, from_property_value=user_to_linear_exp(0.02, 20.0, 3.33), to_property_value=linear_to_user_exp(0.02, 20.0, 3.33))
        self.ir_size_factor_parameter = WrappingParameter(name='Ir Size Factor', parent=self, property_host=self, source_property='ir_size_factor', display_value_conversion=to_percentage_display, from_property_value=user_to_linear_log(0.2, 5.0), to_property_value=linear_to_user_log(0.2, 5.0))
        self.ir_category_list_parameter = EnumWrappingParameter(name='IR Category', parent=self, values_host=self._live_object, index_property_host=self, values_property='ir_category_list', index_property='ir_category_index')
        self.ir_file_list_parameter = EnumWrappingParameter(name='IR', parent=self, values_host=self._live_object, index_property_host=self, values_property='ir_file_list', index_property='ir_file_index')
        self.vintage_enum_copy = NotifyingList(available_values=self.vintage_labels, default_value=0)
        self.vintage_parameter_copy = EnumWrappingParameter(name='Vintage Copy', parent=self, values_host=self.vintage_enum_copy, index_property_host=self.vintage_enum_copy, values_property='available_values', index_property='index')
        self.routing_parameter_enum = NotifyingList(available_values=self.routing_full_labels, default_value=0)
        self.routing_parameter_eq_off = EnumWrappingParameter(name='Routing Eq Off', parent=self, values_host=self.routing_parameter_enum, index_property_host=self.routing_parameter_enum, values_property='available_values', index_property='index')
        self.routing_parameter_eq_on_prealgo_off = EnumWrappingParameter(name='Routing Eq On PreAlgo Off', parent=self, values_host=self.routing_parameter_enum, index_property_host=self.routing_parameter_enum, values_property='available_values', index_property='index')
        self.routing_parameter_eq_on_prealgo_on = EnumWrappingParameter(name='Routing Eq On PreAlgo On', parent=self, values_host=self.routing_parameter_enum, index_property_host=self.routing_parameter_enum, values_property='available_values', index_property='index')
        ...
        self._additional_parameters = (self.band, self.main_section, self.ir_post_processing_bool, self.ir_attack_time_parameter, self.ir_decay_time_parameter, self.ir_size_factor_parameter, self.ir_category_list_parameter, self.ir_file_list_parameter, self.vintage_parameter_copy, self.routing_parameter_eq_off, self.routing_parameter_eq_on_prealgo_on, self.routing_parameter_eq_on_prealgo_off)
        self._additional_options = (self.feedback_ms_sync_switch, self.eq_low_type_switch, self.eq_high_type_switch, self.eq_on_option, self.ir_post_processing_option, self.freeze_option, self.freeze_in_option, self.prealgo_option, self.bass_mono_option)
...
class HybridReverbDeviceComponent(DeviceComponentWithTrackColorViewData):
    ...
    def _adjustment_view_data(self):
        ...
        for parameter in touched_parameters:
            if self.LOW_BAND_PARAMETERS_NAME.search(parameter.name) is not None:
                ...
                pass
                continue
        return {'ActiveBandIndex': active_filter_index, 'LowAndHighBandIsSelected': low_and_high_band_is_selected, 'IrIsActive': ir_is_active, 'EqIsActive': eq_is_active}
    ...
    def _shrink_parameters(self):
        if self._visualisation_visible:
            ...
            return [filter_config.left_index <= index <= filter_config.right_index or filter_config_main.left_index <= index <= filter_config_main.right_index or ir_config.left_index <= index <= ir_config.right_index for index in range(8)]
        else:
            return [False] * 8
