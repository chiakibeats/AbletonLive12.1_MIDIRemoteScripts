...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\timeline_navigation.py
...
def inverse_ease_out(t, degree):
    return 1.0 - (1.0 - t) ** (1.0 / degree) if t < 1.0 else 1.0
...
class Region(namedtuple('Region', ['start', 'end'])):
    ...
    def inside(self, outer):
        if isclose(self.start, outer.start) or self.start > outer.start and (not isclose(self.start, outer.start)):
            return (isclose(self.end, outer.end) or (self.end < outer.end and outer!= self))
...
class TimelineNavigation(EventObject):
    ...
    def regions_of_interest(self):
        pass
        rois = {'timeline': self.timeline_roi, 'focused_object': self.focused_object_roi}
    ...
    def _get_zoom_info_for_focused_object(self):
        ...
        if roi is not None:
            ...
            if is_start and region.start < margin:
                ...
                if not is_start and region.end > self.timeline_region.end - margin:
    ...
    def _add_margin_to_zoomed_region(self, zoom_region, focused_region, margin_type):
        ...
        if focused_region is not None and margin_type!= MarginType.NONE:
            ...
            if zoom_region.start <= position <= zoom_region.end:
                ...
                logger.warning('Focused object not visible. Couldn\'t add margin to zoomed region. %d not in %r' % (position, zoom_region))
        return zoom_region
    ...
    def _get_position_for_identifier(self, identifier):
        roi = self._get_roi_for_object_identifier(identifier)
        if roi is not None:
            return roi.region.start if roi.start_identifier == identifier else roi.region.end
    ...
    def _make_region_for_focused_object(self):
        return self._make_region_from_position_identifier(self._focused_identifier) if self._focused_identifier is not None else Region(0, 0)
...
class SimplerWaveformNavigation(TimelineNavigation, WaveformNavigation):
    ...
    def get_region_in_samples(self, region):
        ...
        return Region(sample.beat_to_sample_time(region.start), sample.beat_to_sample_time(region.end)) if liveobj_valid(sample) and sample.warping else region
    ...
    def object_changed(self, identifier1, identifier2):
        if self.selected_slice_focus in (identifier1, identifier2):
            if self._get_selected_slice_index() == 0 and 'Start' in (identifier1, identifier2):
                return False
        return identifier1!= identifier2
...
class ClipTimelineNavigation(TimelineNavigation):
    ...
    @lazy_attribute
    def additional_regions_of_interest(self):
        return {'start_end_marker': self.make_region_of_interest(start_identifier=self.start_marker_focus, end_identifier=self.loop_end_focus, getter=lambda: (self._clip.positions.start_marker, self._clip.positions.loop_end)), 'loop': self.make_region_of_interest(start_identifier=self.loop_start_focus, end_identifier=self.loop_end_focus, getter=lambda: (self._clip.positions.loop_start, self._clip.positions.loop_end)), 'start_end': self.make_region_of_interest(getter=self._get_start_end_region)}

    @lazy_attribute
    def additional_focusable_object_descriptions(self):
        return {self.start_marker_focus: ObjectDescription(('start_end', 'start_end_marker'), 'start_marker'), self.loop_start_focus: ObjectDescription(('start_end', 'loop'), lambda: 'position' if self._clip.looping else 'start_marker'), self.loop_end_focus: ObjectDescription(('start_end',), 'end_marker')}
    ...
    def object_changed(self, identfier1, identifier2):
        if self.start_marker_focus in (identfier1, identifier2) and self.loop_start_focus in (identfier1, identifier2):
    ...
    def get_region_in_samples(self, region):
        return Region(self._clip.beat_to_sample_time(region.start), self._clip.beat_to_sample_time(region.end)) if self._clip.warping else region
