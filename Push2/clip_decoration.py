...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\clip_decoration.py
...
class ClipPositions(EventObject):
    ...
    def _update_start_end_note_times(self):
        ...
        <mask_9> = list(zip(*[(note.start_time, note.start_time + note.duration) for note in all_notes])) if len(all_notes) > 0 else all_notes
        else:  # inserted
            start_times, end_times = ([self.MAX_TIME], [self.MIN_TIME])
