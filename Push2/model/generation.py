...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\model\generation.py
...
class ModelUpdateNotifier(object):
    def __init__(self, step=None, parent=None, delegate=None):
        ...
        _step = [] if self._step is None else self._step
        else:  # inserted
            self.path = parent.path + [self._step]
    ...
    def to_json(self):
        if self._value is not None:
            if self._value.is_valid():
                res = {}
                for name, wrapper in self.values.items():
...
class ModelMixin(WrapperBase):
    ...
    def to_json(self):
        return dict(((name, obj.to_json()) for name, obj in self.data.items()))
...
class ModelFingerprintVisitor(ModelVisitor):
    ...
    def fingerprint(self):
        if self._fingerprint is None:
            self._fingerprint = ';'.join(('%s(%s)' % (classname, ','.join(property_prints)) for classname, property_prints in sorted(self._class2proplist.items(), key=lambda item: item[0])))
