...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\echo.py
...
class EchoDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        self._channel_names_provider = NotifyingList(available_values=['Left', 'Right'], default_value=EchoDeviceDecorator.EchoChannelName.left)
        ...
        self.channel_switch_lr_option = DeviceSwitchOption(name='L/R Switch', parameter=self.channel_switch_parameter, labels=['Left', 'Right'])
        self.channel_switch_ms_option = DeviceSwitchOption(name='M/S Switch', parameter=self.channel_switch_parameter, labels=['Mid', 'Side'])
    ...
    @property
    def options(self):
        return (self.channel_switch_lr_option, self.channel_switch_ms_option, self.link_option, self.sync_l_option, self.sync_r_option, self.sync_m_option, self.sync_s_option, self.clip_dry_option, self.filter_on_option, self.feedback_inv_option, self.modulation_times_four_option, self.reverb_loc_option, self.duck_option, self.gate_option, self.wobble_option, self.noise_option)

class EchoDeviceComponent(DeviceComponentWithTrackColorViewData):
    TUNNEL_VISUALISATION_CONFIGURATION_IN_BANKS = {0: ButtonRange(0, 3), 1: ButtonRange(2, 5)}
    FILTER_VISUALISATION_CONFIGURATION_IN_BANKS = {0: ButtonRange(4, 5), 3: ButtonRange(1, 4)}
    ...
    def _adjustment_view_data(self):
        ...
        for parameter in touched_parameters:
            if parameter.name == 'Feedback':
                ...
                pass
                continue
        return {'AdjustingTunnelLeft': adjusting_tunnel_left, 'AdjustingTunnelRight': adjusting_tunnel_right, 'AdjustingFilterHighPass': adjusting_filter_hp, 'AdjustingFilterLowPass': adjusting_filter_lp, 'AdjustingLfo': adjusting_lfo, 'AdjustingLfoPhase': adjusting_lfo_phase}
    ...
    def _configuration_view_data(self):
        ...
        return {'TunnelLeft': tunnel_left, 'TunnelRight': tunnel_right, 'FilterLeft': filter_left, 'FilterRight': filter_right, 'LfoLeft': lfo_left, 'LfoRight': lfo_right}
