...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\drift.py
...
class DriftDeviceDecorator(LiveObjectDecorator, EventObject):
    ...
    def __init__(self, *a, **k):
        ...
        self._add_enum_parameter(name='Mod Slot', values=['1', '2', '3'], default_value=self.modSlotSelect.src1)
        self._add_enum_parameter(name='Mod Dest', values=['Shape', 'Pitch', 'Filter', 'LFO'], default_value=self.modDestSelect.shape)
        self.voice_mode = self._add_non_automatable_enum_parameter(name='Voice Mode', list='voice_mode_list', index='voice_mode_index')
        self.voice_count = self._add_non_automatable_enum_parameter(name='Voice Count', list='voice_count_list', index='voice_count_index')
        self._add_non_automatable_enum_parameter(name='Shape Mod Src', list='mod_matrix_shape_source_list', index='mod_matrix_shape_source_index')
        self._add_non_automatable_enum_parameter(name='LFO Mod Src', list='mod_matrix_lfo_source_list', index='mod_matrix_lfo_source_index')
        ...
        self._add_non_automatable_enum_parameter(name='Pitch Mod Src 1', list='mod_matrix_pitch_source_1_list', index='mod_matrix_pitch_source_1_index')
        self._add_non_automatable_enum_parameter(name='Pitch Mod Src 2', list='mod_matrix_pitch_source_2_list', index='mod_matrix_pitch_source_2_index')
        self._add_non_automatable_enum_parameter(name='Mod Source 1', list='mod_matrix_source_1_list', index='mod_matrix_source_1_index')
        self._add_non_automatable_enum_parameter(name='Mod Source 2', list='mod_matrix_source_2_list', index='mod_matrix_source_2_index')
        self._add_non_automatable_enum_parameter(name='Mod Source 3', list='mod_matrix_source_3_list', index='mod_matrix_source_3_index')
        self._add_non_automatable_enum_parameter(name='Mod Dest 1', list='mod_matrix_target_1_list', index='mod_matrix_target_1_index')
        self._add_non_automatable_enum_parameter(name='Mod Dest 2', list='mod_matrix_target_2_list', index='mod_matrix_target_2_index')
        self._add_non_automatable_enum_parameter(name='Mod Dest 3', list='mod_matrix_target_3_list', index='mod_matrix_target_3_index')
        self._add_non_automatable_int_parameter(name='PB Range', property_name='pitch_bend_range', min=0, max=12, units='st')
        self._add_switch_option(name='Osc 2 Wave', pname='Osc 2 Wave', labels=['Sin', 'Tri', 'Sat', 'Saw', 'Rec'])
        self._add_switch_option(name='Env 2 Cyc On', pname='Env 2 Cyc On', labels=['Env', 'Cyc'])
...
class DriftDeviceComponent(DeviceComponentWithTrackColorViewData):
    WAVEFORM_VISUALISATION_CONFIGURATION_IN_BANKS = {0: ButtonRange(1, 3), 1: ButtonRange(1, 3)}
        ...
        if not liveobj_valid(self._decorated_device):
            ...
            for parameter in touched_parameters:
                if parameter.name == 'HP Freq':
                    ...
                    continue
                elif parameter.name in ['Cyc Tilt', 'Cyc Hold', 'Cyc Mode', 'Cyc Rate', 'Cyc Ratio', 'Cyc Time', 'Cyc Synced']:
                    adjusting_cycling_envelope = True
                    continue
                else:
                    if parameter.name in ['Osc 1 Wave', 'Osc 2 Wave', 'Osc 1 Shape', 'Osc 1 Oct', 'Osc 2 Oct', 'Osc 2 Detune', 'Osc 1 Gain', 'Osc 2 Gain', 'Noise Gain']:
                        adjusting_waveform = True
                    pass
                    continue
            return {'AdjustingWaveform': adjusting_waveform, 'AdjustingFilterHighPass': adjusting_filter_hp, 'AdjustingFilterLowPass': adjusting_filter_lp, 'AdjustingCyclingEnvelope': adjusting_cycling_envelope}
    ...
    def _shrink_parameters(self):
        if self._visualisation_visible:
            ...
            return [waveform_config.left_index <= index <= waveform_config.right_index or filter_config.left_index <= index <= env1_config.right_index or env2_config.left_index <= index <= env2_config.right_index for index in range(8)]
        else:
            return [False] * 8
    ...
    def _configuration_view_data(self):
        if not liveobj_valid(self._decorated_device):
            ...
            return {'WaveformLeft': waveform_left, 'WaveformRight': waveform_right, 'FilterLeft': filter_left, 'FilterRight': filter_right, 'Env1Left': env1_left, 'Env1Right': env1_right, 'Env2Left': env2_left, 'Env2Right': env2_right}
    ...
    def _envelope_visualisation_data(self):
        ...
        return {'EnvelopeShow': make_vector(shown_features), 'Envelope1Focus': make_vector(focused_features1), 'Envelope2Focus': make_vector(focused_features2)}
