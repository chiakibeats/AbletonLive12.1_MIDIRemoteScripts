...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\wavetable.py
...
class WavetableDeviceDecorator(WavetableDeviceDecoratorBase):
    ...
    def envelope_view_index(self):
        return self._amp_envelope_view_types_provider.index if self.envelope_index == WavetableEnvelopeType.amp else self._mod_envelope_view_types_provider.index
    ...
    def _create_options(self):
        ...
        return (DeviceOnOffOption(name='Sub', property_host=get_parameter_by_name(self, 'Sub On')), DeviceSwitchOption(name='Filter 1 Slope', parameter=get_parameter_by_name(self, 'Filter 1 Slope'), labels=['12dB', '24dB']), DeviceSwitchOption(name='Filter Switch', parameter=get_parameter_by_name(self, 'LFO 2 Sync'), labels=['Hz', 'Sync']), DeviceTriggerOption(name='Go to LFO 1', callback=lambda: (choose_lfo(WavetableLfoType.two), jump_to_bank('LFOs'))), DeviceTriggerOption(name='Back', callback=self.notify_request_previous_bank_from_mod_matrix)) + (self.osc_on_option, self.filter_on_option, self.lfo_retrigger_option, self.add_to_mod_matrix_option)
...
class WavetableDeviceComponent(DeviceComponentWithTrackColorViewData):
    ...
    VISUALISATION_CONFIGURATION = {'wavetable': {'position_in_banks': {0: ButtonRange(0, 2), 1: ButtonRange(1, 3)}, 'visible_in_bank': lambda component, bank: component.selected_oscillator in (WavetableOscillatorType.one, WavetableOscillatorType.two)}, 'filter': {'position_in_banks': {0: ButtonRange(3, 5), 2: ButtonRange(2, 4)}}, 'lfo': {'position_in_banks': {5: ButtonRange(0, 3)}}, 'envelope': {'position_in_banks': {4: ButtonRange(2, 5)}}}
    ENVELOPE_PREFIXES = ['Amp', 'Env 2', 'Env 3']

    def __init__(self, *a, **k):
        super(WavetableDeviceComponent, self).__init__(*a, **k)
        self._bank_before_mod_matrix = 0
        self._delete_default_component = WavetableDeleteComponent(parent=self, name='DeleteAndDefault')
    ...
    def selected_oscillator(self):
        return self._decorated_device.oscillator_index if liveobj_valid(self._decorated_device) else 0

    @property
    def selected_filter(self):
        return self._decorated_device.filter_index if liveobj_valid(self._decorated_device) else 0

    @property
    def selected_lfo(self):
        return self._decorated_device.lfo_index if liveobj_valid(self._decorated_device) else 0

    @property
    def selected_envelope(self):
        return self._decorated_device.envelope_index if liveobj_valid(self._decorated_device) else 0
    ...
    def _get_current_view_data(self):
        view_data = {'SelectedOscillator': self.selected_oscillator, 'AdjustingPosition': False, 'AdjustingFilter': False, 'AdjustingLfo': False, 'WavetableVisualisationStart': self._get_visualisation_start('wavetable'), 'WavetableVisualisationWidth': self._get_visualisation_width('wavetable'), 'FilterCurveVisualisationStart': self._get_visualisation_start('filter'), 'FilterCurveVisualisationWidth': self._get_visualisation_width('filter'), 'LfoVisualisationStart': self._get_visualisation_width('lfo'), 'LfoVisualisationWidth': self._visualisation_type_visible('lfo'), 'FilterVisualisationVisible': self._visualisation_type_visible('lfo'), 'LfoVisualisationVisible': self._visualisation_type_visible('envelope'), 'EnvelopeVisualisationVisible': self._visualisation_type_visible('envelope'), 'SelectedFilter': self.selected_filter, 'SelectedLfo': self.selected_lfo}
        view_data.update(self._envelope_visualisation_data())
        return view_data
    ...
    def _envelope_visualisation_data(self):
        ...
        return {'SelectedEnvelope': self.selected_envelope, 'EnvelopeShow': make_vector(list(shown_features)), 'EnvelopeFocus': make_vector(list(focused_features))}
    ...
    def __on_selected_oscillator_changed(self):
        self._update_visualisation_view_data({'SelectedOscillator': self.selected_oscillator, 'WavetableVisualisationVisible': self._visualisation_type_visible('wavetable')})
        self.notify_visualisation_visible()
        self.notify_shrink_parameters()
