...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push2\push2.py
...
def make_dialog_layer(priority=consts.DIALOG_PRIORITY, *a, **k):
    return (BackgroundLayer('global_param_controls', 'select_buttons', 'track_state_buttons', priority=priority), Layer(*a, priority=priority, **k))
...
class MixModeBehaviour(ReenterBehaviour):
    ...
    def press_immediate(self, component, mode):
        if self._push._is_on_master() and self._push._mix_modes.selected_mode != 'track':
...
class Push2(IdentifiableControlSurface, PushBase):
    ...
    def _create_note_settings_component_layer(self):
        return Layer(full_velocity_button='accent_button', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _create_capture_and_insert_scene_component(self):
        return CaptureAndInsertSceneComponent(name='Capture_And_Insert_Scene', decorator_factory=self._clip_decorator_factory)
    ...
    def _create_loop_navigation_layer(self):
        return Layer(prev_loop_page_button='page_left_button', next_loop_page_button='page_right_button')
    ...
    def _create_session_mode(self):
        ...
        simple_mode_switcher = SimpleModeSwitcher(session_modes, is_enabled=False, layer=Layer(cycle_button=self.note_layout_button, lock_button=ComboElement(self.note_layout_button, modifier='shift_button')))
    ...
    def _create_device_mode(self):
        ...
        return [partial(self._view_control.show_view, 'Detail/DeviceChain'), self._device_component, self._device_or_pad_parameter_chooser, self._setup_freeze_aware_device_navigation(), self._device_note_editor_mode, SetAttributeMode(obj=self._note_editor_settings_component, attribute='parameter_provider', value=self._device_component), self._clip_phase_enabler]
    ...
    def _create_device_navigation(self):
        ...
        device_navigation.chain_selection.layer = Layer(select_buttons='select_buttons', priority=consts.DIALOG_PRIORITY)
        device_navigation.chain_selection.scroll_left_layer = Layer(button='select_buttons_raw[0]', priority=consts.DIALOG_PRIORITY)
        device_navigation.chain_selection.scroll_right_layer = Layer(button='select_buttons_raw[-1]', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.layer = Layer(option_buttons='track_state_buttons', select_buttons='select_buttons', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.scroll_left_layer = Layer(button='select_buttons_raw[0]', priority=consts.DIALOG_PRIORITY)
        device_navigation.bank_selection.scroll_right_layer = Layer(button='select_buttons_raw[-1]', priority=consts.DIALOG_PRIORITY)
    ...
    def _init_session_ring(self):
        self._session_ring = SessionRingTrackProvider(name='Session_Ring', num_tracks=NUM_TRACKS, num_scenes=NUM_SCENES, is_enabled=True)
    ...
    def _create_main_mixer_modes(self):
        ...
        self._main_modes.add_mode('mix', [self._mix_modes, SetAttributeMode(obj=self._note_editor_settings_component, attribute='parameter_provider', value=self._track_parameter_provider), self._clip_phase_enabler], behaviour=MixModeBehaviour(self))
    ...
    def _create_scales_enabler(self):
        return ScalesEnabler(enter_dialog_mode=self._enter_dialog_mode, exit_dialog_mode=self._exit_dialog_mode, is_enabled=False, layer=Layer(toggle_button='scale_presets_button'))
    ...
    def _create_clip_mode(self):
        base_loop_layer = Layer(shift_button='shift_button', loop_button='track_state_buttons_raw[1]', crop_button='track_state_buttons_raw[2]')
        ...
        self._clip_control.mode_selector.add_mode('midi', [make_freeze_aware(self._clip_control.midi_loop_controller, base_loop_layer + Layer(encoders=self.elements.global_param_controls.submatrix[1:4, :], zoom_encoder='fine_grain_param_controls_raw[0]')), self._clip_control.midi_clip_controller])
        self._clip_control.mode_selector.add_mode('audio', [make_freeze_aware(self._clip_control.audio_loop_controller, base_loop_layer + Layer(encoders=self.elements.global_param_controls.submatrix[1:4, :], zoom_encoder='fine_grain_param_controls_raw[0]')), make_freeze_aware(self._clip_control.audio_clip_controller, audio_clip_layer)])
        ...
        instrument_components = (self._drum_component, self._instrument.instrument, self._selected_note_instrument, self._slicing_component)
        ...
        paginators = (self._drum_step_sequencer.paginator, self._instrument.paginator, self._split_melodic_sequencer.paginator, self._slicing_step_sequencer.paginator)
        for paginator in paginators:
            self._clip_control.midi_clip_controller.add_paginator(paginator)
        sequencers = (self._drum_step_sequencer, self._instrument, self._slicing_step_sequencer, self._split_melodic_sequencer)
        ...
        return [partial(self._view_control.show_view, 'Detail/Clip'), self._clip_control, self._clip_phase_enabler]

    def _init_quantize_actions(self):
        self._quantize = self._for_non_frozen_tracks(QuantizationComponent(name='Selected_Clip_Quantize', quantization_names=QUANTIZATION_NAMES_UNICODE, settings_class=self.quantization_settings_class, is_enabled=False, layer=Layer(action_button='quantize_button')))
    ...
    def _init_fixed_length(self):
        ...
        self._fixed_length.settings_component.layer = make_dialog_layer(length_option_buttons='select_buttons', fixed_length_toggle_button='track_state_buttons_raw[0]', legato_launch_toggle_button='track_state_buttons_raw[7]', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _init_setup_component(self):
        ...
        setup.general.layer = Layer(workflow_encoder='parameter_controls_raw[0]', display_brightness_encoder='parameter_controls_raw[1]', led_brightness_encoder='parameter_controls_raw[2]', aftertouch_mode_encoder='parameter_controls_raw[3]', priority=consts.SETUP_DIALOG_PRIORITY)
    ...
    def _create_pad_sensitivity_update(self):
        ...
        self._pad_sensitivity_update.set_profile('drums', PadSettings(sensitivity=playing_profile, aftertouch_enabled=MONO_AFTERTOUCH_ENABLED))
        self._pad_sensitivity_update.set_profile('instrument', PadSettings(sensitivity=playing_profile, aftertouch_enabled=MONO_AFTERTOUCH_ENABLED))
        self._pad_sensitivity_update.set_profile('loop', PadSettings(sensitivity=loop_selector_profile, aftertouch_enabled=MONO_AFTERTOUCH_DISABLED))
