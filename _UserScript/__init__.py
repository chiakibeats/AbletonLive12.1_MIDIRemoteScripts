...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_UserScript\__init__.py
...
map_modes = {k: v for k, v in vars(MapMode).items() if k[:1] != '_' and (not callable(v))}
...
def create_instance(c_instance, user_path=''):
    ...
    if user_path != '':
        ...
        if file_object:
            ...
            volume_controls = [(-1, -1)] * 8
            trackarm_controls = [-1] * 8
            bank_controls = {'ONOFF': -1, 'TOGGLELOCK': -1, 'NEXTBANK': -1, 'PREVBANK': -1, 'BANK1': -1, 'BANK2': -1, 'BANK3': -1, 'BANK4': -1, 'BANK5': -1, 'BANK6': -1, 'BANK7': -1, 'BANK8': -1}
            controller_descriptions = {'CHANNEL': -1}
            mixer_options = {'NUMSENDS': 2, 'INVERTMUTELEDS': True, 'SEND1': [-1] * 8, 'SEND2': [-1] * 8, 'MUTE': [-1] * 8, 'SOLO': [-1] * 8, 'SELECT': [-1] * 8, 'SENDMAPMODE': [-1] * 8, 'MASTERVOLUME': map_modes['Absolute'], 'MASTERVOLUMECHANNEL': -1, 'CUEVOLUME': -1, 'CUEVOLUMECHANNEL': -1, 'CROSSFADER': -1, 'CROSSFADERCHANNEL': -1, 'CROSSFADERMAPMODE': map_modes['Absolute']}
                ...
                for pad in range(16):
                    ...
                    if config_parser.has_option('Globals', option_name):
                        ...
                        if note in range(128):
                            ...
                            if channel == -1 and controller_descriptions['CHANNEL'] != -1:
            ...
            if config_parser.has_section('MixerControls'):
                ...
                if config_parser.has_option('MixerControls', 'MixerButtonsToggle') and (not config_parser.getboolean('MixerControls', 'MixerButtonsToggle')):
            ...
            for index in range(8):
                ...
                if config_parser.has_section('MixerControls'):
                    volume_controls[index] = tuple([parse_identifier('MixerControls', 'VolumeSlider{}'.format(friendly_index)), parse_channel('MixerControls', 'Slider{}Channel'.format(friendly_index))])
        ...
        pass
    return type(get_name_for_script(user_path), (GenericScript,), {})(c_instance, device_map_mode, volume_map_mode, tuple(device_controls), transport_controls, tuple(volume_controls), tuple(trackarm_controls), bank_controls, controller_descriptions, mixer_options, device_component_class=DeviceComponent)