...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launch_Control\LaunchControl.py
...
def make_all_encoders(name_prefix='', make_encoder=make_launch_control_encoder):
    return ([make_encoder(41 + index, name_prefix + '_Bottom_Encoder_' + str(index)) for index in range(8)], [make_encoder(21 + index, name_prefix + '_Top_Encoder_' + str(index)) for index in range(8)])
pad_identifiers = [9 + i if i < 4 else 21 + i for i in range(8)]
CC_STATUS = 176
MODE_SYSEX_MAP = {'mixer': Sysex.MIXER_MODE, 'session': Sysex.SESSION_MODE, 'device': Sysex.DEVICE_MODE}
...
class LaunchControl(ControlSurface):
    ...
    def _init_modes(self):
        ...
        self._modes.add_mode('mixer', [partial(self._session.set_mixer, self._mixer), LayerMode(self._session, self._mixer_track_nav_layer), self._mixer, self._session, self._show_controlled_tracks_message])
        self._modes.add_mode('session', [partial(self._session.set_mixer, self._session_mixer), LayerMode(self._session, self._session_layer), self._session_mixer, self._session, self._show_controlled_tracks_message])
    ...
    def _show_controlled_tracks_message(self):
        start = self._session.track_offset()
