...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SessionComponent.py
...
class SessionComponent(CompoundComponent):
    ...
    def _on_next_scene_value(self, value):
        if self.is_enabled() and (value!= 0 or not self._next_scene_button.is_momentary()):
                selected_scene = self.song().view.selected_scene
    ...
    def _on_prev_scene_value(self, value):
        if self.is_enabled() and (value!= 0 or not self._prev_scene_button.is_momentary()):
                selected_scene = self.song().view.selected_scene
                all_scenes = self.song().scenes
