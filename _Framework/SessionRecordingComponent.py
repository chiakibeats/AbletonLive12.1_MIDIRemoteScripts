...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\SessionRecordingComponent.py
...
class SessionRecordingComponent(CompoundComponent):
    ...
    def _update_generic_new_button(self, new_button):
        if new_button and self.is_enabled():
                ...
                return clip_slot!= None and clip_slot.clip or None
                    can_new = selected_track.can_be_armed and selected_track.playing_slot_index >= 0
    ...
    def _on_delete_automation_value(self, value):
        if not self.is_enabled() or value:
                ...
                if not clip or not track_frozen:
    ...
    def _on_record_button_value(self, value):
        if self.is_enabled() and value and (not self._stop_recording()):
