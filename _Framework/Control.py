...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Framework\Control.py
...
class ButtonControl(Control):
    ...
    class State(Control.State):
        ...
        def _press_button(self):
            ...
            if self._notifications_enabled() and (not is_pressed) and (self._pressed_listener is not None):
                        if self._repeat:
        ...
        def _release_button(self):
            is_pressed = self._is_pressed
            self._is_pressed = False
            if self._notifications_enabled() and is_pressed and (self._released_listener is not None):
