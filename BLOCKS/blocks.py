...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\BLOCKS\blocks.py
...
class Blocks(SimpleControlSurface):
    ...
    def _create_controls(self):
        ...
        self._session_matrix = ButtonMatrixElement(rows=[[self._pads_raw[offset + col] for col in range(NUM_TRACKS)] for offset in range(80, 49, -10)], name='Session_Matrix')
        self._scene_launch_button_matrix = ButtonMatrixElement(rows=[[self._pads_raw[identifier] for identifier in range(89, 58, -10)]], name='Scene_Launch_Button_Matrix')
        self._stop_all_clips_button = ButtonElement(True, MIDI_NOTE_TYPE, 0, 127, skin=skin, name='Stop_All_Clips_Button')
        ...
        self._mode_cycle_button = ButtonElement(True, MIDI_CC_TYPE, 0, 127, skin=skin, name='Mode_Button')
        self._drum_pads = ButtonMatrixElement(rows=[[self._pads_raw[offset + col] for col in range(4)] for offset in range(48, 35, -4)], name='Drum_Pads')
        self._tempo_encoder = EncoderElement(MIDI_PB_TYPE, 0, 0, Live.MidiMap.MapMode.absolute, send_should_depend_on_forwarding=False, name='Tempo_Encoder')
        self._tempo_encoder.reset = nop
        self._sysex_element = SysexElement(sysex_identifier=SYSEX_HEADER, name='Sysex_Element')
        self._sysex_element.add_value_listener(nop)
        self._surface_update_message_element = ButtonElement(True, MIDI_CC_TYPE, 0, 64, name='Surface_Update_Message_Element')
        self.__on_surface_update_message_received.subject = self._surface_update_message_element

    def _create_session(self):
        self._session_ring = SessionRingComponent(num_tracks=NUM_TRACKS, num_scenes=NUM_SCENES, is_enabled=False, name='Session_Ring')
    ...
    def _create_modes(self):
        ...
        self._note_modes.add_mode('drum', [partial(self._send_mode_message, 'drum'), LayerMode(self._drum_group, Layer(matrix=self._drum_pads))])
        self._note_modes.add_mode('melodic', [partial(self._send_mode_message, 'melodic'), self._melodic_pad_translator])
        self._note_modes.add_mode('disabled', [partial(self._send_mode_message, 'disabled')])
        self._modes = ModesComponent(name='Modes')
        self._modes.add_mode('session', [partial(self._send_mode_message, 'session'), partial(self._clear_send_cache, self._pads_raw), LayerMode(self._session, Layer(clip_launch_buttons=self._session_matrix, scene_launch_buttons=self._scene_launch_button_matrix, stop_all_clips_button=self._stop_all_clips_button)), LayerMode(self._session_navigation, Layer(down_button=self._nav_down_button, up_button=self._nav_up_button, left_button=self._nav_left_button, right_button=self._nav_right_button))])
