...
class LaunchkeyElements(object):
    ...
    def __init__(self, *a, **k):
        ...
        self.scene_launch_buttons_raw = [create_button(104, 'Scene_Launch_Button', channel=0), create_button(105, 'Stop_Solo_Mute_Button', channel=0)]
        self.scene_launch_buttons = ButtonMatrixElement(rows=[self.scene_launch_buttons_raw], name='Scene_Launch_Buttons')
        self.clip_launch_matrix = ButtonMatrixElement(rows=[create_button(offset + col_index, '{}_Clip_Launch_Button_{}'.format(col_index, row_index), msg_type=MIDI_NOTE_TYPE, channel=0) for col_index in enumerate(range(96, 119, 16))], name='Clip_Launch_Matrix')
        drum_pad_rows = ((48, 49, 50, 51), (44, 45, 46, 47), (40, 41, 42, 43), (36, 37, 38, 39))
        self.drum_pads = ButtonMatrixElement(rows=[create_button(row_identifiers[col_index], 'Drum_Pad_{}_{}'.format(col_index, row_index), msg_type=MIDI_NOTE_TYPE, channel=DRUM_CHANNEL) for col_index in enumerate(drum_pad_rows)], name='Drum_Pads')
        self.pots = ButtonMatrixElement(rows=[[create_encoder(index + 21, 'Pot_{}'.format(index)) for index in range(SESSION_WIDTH)]], name='Pots')
