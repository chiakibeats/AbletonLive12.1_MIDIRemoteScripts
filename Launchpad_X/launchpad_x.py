...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_X\launchpad_x.py
...
class Launchpad_X(InstrumentControlMixin, NovationBase):
    ...
    def _create_mixer_modes(self):
        self._mixer_modes = ModesComponent(name='Mixer_Modes', is_enabled=False, enable_skinning=True, layer=Layer(volume_button=self._elements.scene_launch_buttons_raw[0], pan_button=self._elements.scene_launch_buttons_raw[1], send_a_button=self._elements.scene_launch_buttons_raw[2], send_b_button=self._elements.scene_launch_buttons_raw[3], stop_button=self._elements.scene_launch_buttons_raw[4], mute_button=self._elements.scene_launch_buttons_raw[5], solo_button=self._elements.scene_launch_buttons_raw[6], arm_button=self._elements.scene_launch_buttons_raw[7]))
        ...
        def add_fader_mode(name, color, is_pan=False):
            ...
            self._mixer_modes.add_mode(name, (partial(self._elements.button_fader_setup_element.send_value, sysex.FADER_HORIZONTAL_ORIENTATION if is_pan else sysex.FADER_VERTICAL_ORIENTATION if is_pan else sysex.FADER_UNIPOLAR, sysex.FADER_BIPOLAR if is_pan else sysex.FADER_UNIPOLAR), partial(self._mixer.set_static_color_value, color), self._clear_send_cache_of_button_fader_color_elements, AddLayerMode(self._mixer, Layer(**control_dict)), button_fader_layout_mode), behaviour=ReenterBehaviour(on_reenter=select_none_mode))
    ...
    def _create_note_modes(self):
        self._drum_group = DrumGroupComponent(name='Drum_Group', is_enabled=False, translation_channel=DRUM_FEEDBACK_CHANNEL, layer=Layer(matrix='drum_pads', scroll_up_button='left_button', scroll_down_button='right_button', scroll_page_up_button='up_button', scroll_page_down_button='down_button'))
        ...
        self._note_modes.add_mode('scale', (self._scale_pad_translator, AddLayerMode(self._background, layer=Layer(up_button='up_button', down_button='down_button', left_button='left_button', right_button='right_button'))))
    ...
    def _create_main_modes(self):
        self._main_modes = ModesComponent(name='Main_Modes', is_enabled=False, layer=Layer(session_button='session_mode_button', note_button='note_mode_button', custom_button='custom_mode_button'))
        self._main_modes.add_mode('session', self._session_modes, behaviour=ImmediateBehaviour())
        self._main_modes.add_mode('note', self._note_modes, behaviour=ImmediateBehaviour())
