...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_MK2\Launchpad_MK2.py
...
USER_1_MATRIX_IDENTIFIERS = [[64, 65, 66, 67, 96, 97, 98, 99], [60, 61, 62, 63, 92, 93, 94, 95], [56, 57, 58, 59, 88, 89, 90, 91], [52, 53, 54, 55, 84, 85, 86, 87], [48, 49, 50, 51, 80, 81, 82, 83], [44, 45, 46, 47, 76, 77, 78, 79], [40, 41, 42, 43, 72, 73, 74, 75], [36, 37, 38, 39, 68, 69, 70, 71]]
USER_1_CHANNEL = 5
USER_2_CHANNEL = 13
...
class Launchpad_MK2(IdentifiableControlSurface, OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._user_1_button = FilteringMultiElement([make_button(109, channel, msg_type=MIDI_CC_TYPE, name='User_1_Mode_Button_ch_%d' % (channel,)) for channel in [0] + multi_button_channels], feedback_channels=[0, 5, 6, 7])
        self._user_2_button = FilteringMultiElement([make_button(110, channel, msg_type=MIDI_CC_TYPE, name='User_2_Mode_Button_ch_%d' % (channel,)) for channel in [0] + multi_button_channels], feedback_channels=[0, 13, 14, 15])
        ...
        self._session_matrix_raw = [make_button(col + offset, 0, name='Session_Matrix_Button_%d_%d' % (col, row)) for col in enumerate(range(81, 10, -10))]
    ...
    def _create_user_controls(self):
        pass
        self._user_1_matrix = ButtonMatrixElement(rows=[make_button(identifier, USER_1_CHANNEL, name='User_1_Button_%d_%d' % (row_index, col_index)) for col_index, identifier in enumerate(row)], name='User_1_Matrix')
        self._user_1_arrow_buttons = ButtonMatrixElement(rows=[[make_button(identifier, USER_1_CHANNEL, msg_type=MIDI_CC_TYPE, name='User_1_Arrow_Button_%d' % (index,)) for index, identifier in enumerate(range(104, 108))]], name='User_1_Arrow_Buttons')
        self._user_1_side_buttons = ButtonMatrixElement(rows=[[make_button(identifier, USER_1_CHANNEL, name='User_1_Side_Button_%d' % (index,))] for index, identifier in enumerate(range(100, 108))], name='User_1_Side_Buttons')
        self._user_2_matrix = ButtonMatrixElement(rows=[make_button(offset + col_index, USER_2_CHANNEL, name='User_2_Button_%d_%d' % (col_index, row_index)) for col_index in enumerate(range(81, 10, -10))], name='User_2_Matrix')
        self._user_2_arrow_buttons = ButtonMatrixElement(rows=[[make_button(identifier, USER_2_CHANNEL, msg_type=MIDI_CC_TYPE, name='User_2_Arrow_Button_%d' % (index,)) for index, identifier in enumerate(range(104, 108))]], name='User_2_Arrow_Buttons')
        self._user_2_side_buttons = ButtonMatrixElement(rows=[[make_button(identifier, USER_2_CHANNEL, name='User_2_Side_Button_%d' % (index,))] for index, identifier in enumerate(range(89, 18, -10))], name='User_2_Side_Buttons')
    ...
    def _create_modes(self):
        ...
        self._modes.add_mode('send_a_mode', [self._session, self._create_translating_mixer_background(consts.SEND_A_MODE_CHANNEL), self._mixer_send_a_layer], layout_byte=6, groups=set('mixer'))
        self._modes.add_mode('send_b_mode', [self._session, self._create_translating_mixer_background(consts.SEND_B_MODE_CHANNEL), self._mixer_send_b_layer], layout_byte=7, groups=set('mixer'))
