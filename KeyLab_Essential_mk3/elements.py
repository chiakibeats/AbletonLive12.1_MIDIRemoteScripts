...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\KeyLab_Essential_mk3\elements.py
...
def create_rgb_button(identifier, name=None, **k):
    return create_sysex_sending_button(identifier, name, midi.LED_HEADER + (midi.BUTTON_ID_TO_SYSEX_ID[identifier],), is_rgb=True, **k)
...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_matrix([[40, 41, 42, 43], [36, 37, 38, 39]], 'Pad_Bank_A', element_factory=create_rgb_pad, channels=10)
        self.add_matrix([range(44, 52)], 'Pad_Bank_B', element_factory=create_rgb_pad, channels=10)
        ...
        self.add_matrix([list(range(96, 104)) + list(range(105, 113))], 'Continuous_Controls', element_factory=RealigningEncoderElement)
        ...
        self.add_sysex_element(midi.DISPLAY_HEADER, 'Display_Full_Screen_Command', send_message_generator=CachingSendMessageGenerator(midi.make_full_screen_message), optimized=True)
        self.display_line_1 = DisplayLineElement(name='Display_Line_1', display_fn=lambda message: self.display_full_screen_command.send_value(line1=message), default_formatting=Line1Text())
        self.display_line_2 = DisplayLineElement(name='Display_Line_2', display_fn=lambda message: self.display_full_screen_command.send_value(line2=message), default_formatting=Line2Text())
        self.add_sysex_element(midi.DISPLAY_HEADER + (1,), 'Display_Header_Command', send_message_generator=lambda text: midi.make_full_screen_message(1, (0,), text), optimized=True)
        self.add_sysex_element(midi.DISPLAY_HEADER + (3,), 'Display_Footer_Command', send_message_generator=lambda icon1, icon2, icon3, icon4: midi.DISPLAY_HEADER + (3,) + (16, icon1.state.value, 0, 18, icon1.type.value, 0) + (32, icon2.state.value, 0, 34, icon2.type.value, 0) + (48, icon3.state.value, 0, 50, icon3.type.value, 0) + (64, icon4.state.value, 0, 66, icon4.type.value, 0) + (midi.SYSEX_END,), optimized=True)
        self.add_sysex_element(midi.DISPLAY_HEADER + (23,), 'Display_Popup_Command', send_message_generator=partial(midi.make_full_screen_message, 23), optimized=True)
        self.display_parameter_commands = []
        for i, element in enumerate(self.continuous_controls_raw + [self.encoder_9, self.fader_9]):
            self.add_sysex_element(midi.DISPLAY_HEADER + (32, i), 'Display_Parameter_Command_{}'.format(i), send_message_generator=partial(midi.make_full_screen_message, line3=element.original_identifier()), optimized=True)
