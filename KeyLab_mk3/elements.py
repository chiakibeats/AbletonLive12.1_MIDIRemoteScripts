...
class Elements(ElementsBase):
    ...
    def __init__(self, *a, **k):
        ...
        self.add_modified_control(control=self.display_encoder, modifier=self.context_button_0)
        self.add_modified_control(control=self.display_encoder, modifier=self.context_button_1)
        ...
        self.add_sysex_element(midi.DISPLAY_HEADER + (ScreenId.ENCODER_PARAMETER,), 'Display_Parameter_Command', send_message_generator=midi.make_parameter_screen)
        for i in range(8):
            self.add_sysex_element(midi.DISPLAY_HEADER + (i + 1,), 'Display_Button_Command_{}'.format(i), send_message_generator=partial(midi.make_button_segment, i + 1), optimized=True)
