...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_Komplete_Kontrol\komplete_kontrol_base.py
...
class KompleteKontrolBase(SimpleControlSurface):
    ...
    def _create_controls(self):
        ...
        self._volume_encoders = ButtonMatrixElement(rows=[[create_encoder(index + 80, 'Volume_Encoder_{}'.format(index), is_s_mk2=self.is_s_mk2) for index in range(NUM_TRACKS)]], name='Volume_Encoders')
        self._pan_encoders = ButtonMatrixElement(rows=[[create_encoder(index + 88, 'Pan_Encoder_{}'.format(index), is_s_mk2=self.is_s_mk2) for index in range(NUM_TRACKS)]], name='Pan_Encoders')
        self._track_name_displays = ButtonMatrixElement(rows=[[create_display_line(sysex.TRACK_NAME_DISPLAY_HEADER, index, 'Track_Name_Display_{}'.format(index), width=33 if self.is_s_mk2 else 11) for index in range(NUM_TRACKS)]], name='Track_Name_Displays')
        self._track_volume_displays = ButtonMatrixElement(rows=[[create_display_line(sysex.TRACK_VOLUME_DISPLAY_HEADER, index, 'Track_Volume_Display_{}'.format(index), width=12) for index in range(NUM_TRACKS)]], name='Track_Volume_Displays')
        self._track_panning_displays = ButtonMatrixElement(rows=[[create_display_line(sysex.TRACK_PANNING_DISPLAY_HEADER, index, 'Track_Panning_Display_{}'.format(index), width=12) for index in range(NUM_TRACKS)]], name='Track_Panning_Displays')
        self._track_type_displays = ButtonMatrixElement(rows=[[create_sysex_element(sysex.TRACK_TYPE_DISPLAY_HEADER, index, 'Track_Type_Display_{}'.format(index)) for index in range(NUM_TRACKS)]], name='Track_Type_Displays')
        self._track_mute_displays = ButtonMatrixElement(rows=[[create_sysex_element(sysex.TRACK_MUTE_DISPLAY_HEADER, index, 'Track_Mute_Display_{}'.format(index)) for index in range(NUM_TRACKS)]], name='Track_Mute_Displays')
        self._track_solo_displays = ButtonMatrixElement(rows=[[create_sysex_element(sysex.TRACK_SOLO_DISPLAY_HEADER, index, 'Track_Solo_Display_{}'.format(index)) for index in range(NUM_TRACKS)]], name='Track_Solo_Displays')
        self._track_muted_via_solo_displays = ButtonMatrixElement(rows=[[create_sysex_element(sysex.TRACK_MUTED_VIA_SOLO_DISPLAY_HEADER, index, 'Track_Muted_via_Solo_Display_{}'.format(index)) for index in range(NUM_TRACKS)]], name='Track_Muted_via_Solo_Displays')
        self._track_selection_displays = ButtonMatrixElement(rows=[[create_sysex_element(sysex.TRACK_SELECT_DISPLAY_HEADER, index, 'Track_Selection_Display_{}'.format(index)) for index in range(NUM_TRACKS)]], name='Track_Selection_Displays')
        self._focus_follow_control = SysexElement(lambda value: sysex.TRACK_CHANGED_DISPLAY_HEADER + value + (midi.SYSEX_END,), name='Focus_Follow_Control')
    ...
    def _create_mixer(self):
        self._session_ring = SelectionLinkedSessionRingComponent(name='Session_Ring', num_tracks=NUM_TRACKS, tracks_to_use=partial(tracks_to_use_from_song, self.song), always_snap_track_offset=True)
        self._mixer = self.mixer_component_class(name='Mixer', tracks_provider=self._session_ring, track_assigner=SimpleTrackAssigner(), channel_strip_component_type=self.channel_strip_component_class, is_enabled=False, layer=self._create_mixer_component_layer())
    ...
    def _create_session_recording(self):
        self._session_recording = SessionRecordingComponent(name='Session_Recording', is_enabled=False, layer=Layer(automation_button=self._automation_button))

    def _create_undo_redo(self):
        self._undo_redo = UndoRedoComponent(name='Undo_Redo', is_enabled=False, layer=Layer(undo_button=self._undo_button, redo_button=self._redo_button))

    def _create_clip_launch(self):
        self._clip_launch = ClipLaunchComponent(name='Clip_Launch', is_enabled=False, layer=Layer(clip_launch_button=self._clip_launch_button, track_stop_button=self._track_stop_button))

    def _create_clip_launch_background(self):
        self._clip_launch_background = BackgroundComponent(name='Background', is_enabled=False, add_nop_listeners=True, layer=Layer(clip_launch_button=self._clip_launch_button, track_stop_button=self._track_stop_button, priority=-1))

    def _create_detail_clip(self):
        self._detail_clip = DetailClipComponent(name='Detail_Clip', is_enabled=False, layer=Layer(quantize_notes_button=self._quantize_button, delete_notes_button=self._clear_button))

    def _create_focus_follow(self):
        self._focus_follow = FocusFollowComponent(name='Focus_Follow', is_enabled=False, layer=Layer(focus_follow_control=self._focus_follow_control))
    ...
    def _create_view_based_modes(self):
        ...
        self._view_based_modes.add_mode('session', (EnablingMode(self._clip_launch), AddLayerMode(self._transport, Layer(session_record_button=self._record_button, record_button=self._count_in_button))))
        self._view_based_modes.add_mode('arrange', AddLayerMode(self._transport, Layer(record_button=self._record_button, session_record_button=self._count_in_button)))
