...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Code_Series\code.py
...
class Code(SimpleControlSurface):
    ...
    def _create_controls(self):
        ...
        self._faders = ButtonMatrixElement(rows=[[make_slider(index, 'Fader_%d' % (index + 1,)) for index in range(8)]], name='Faders')
        self._master_fader = make_slider(8, 'Master_Fader')
        self._encoders = ButtonMatrixElement(rows=[[make_encoder(index + 16, 'Encoder_%d' % (index + 1,)) for index in range(8)]], name='Encoders')
        self._track_select_buttons = ButtonMatrixElement(rows=[[make_button(index + 24, 'Track_Select_Button_%d' % (index + 1,)) for index in range(8)]], name='Track_Select_Buttons')
        self._mute_buttons = ButtonMatrixElement(rows=[[make_button(index + 8, 'Mute_Button_%d' % (index + 1,)) for index in range(8)]], name='Mute_Buttons')
        self._solo_buttons = ButtonMatrixElement(rows=[[make_button(index + 16, 'Solo_Button_%d' % (index + 1,)) for index in range(8)]], name='Solo_Buttons')
        self._arm_buttons = ButtonMatrixElement(rows=[[make_button(index, 'Record_Arm_Button_%d' % (index + 1,)) for index in range(8)]], name='Record_Arm_Buttons')
