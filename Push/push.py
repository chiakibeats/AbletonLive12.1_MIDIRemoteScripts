...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Push\push.py
...
class Push(PushBase):
    ...
    def _create_settings(self):
        ...
        self.__on_aftertouch_mode.subject = settings['aftertouch_mode']
        return settings
    ...
    def _on_handshake_success(self):
        ...
        self.__on_aftertouch_mode(self._settings['aftertouch_mode'].value)
    ...
    def _create_main_mixer_modes(self):
        ...
        self._main_modes.add_mode('track', [self._track_modes, self._track_mixer, (self._mixer, self._mixer_track_layer), self._track_note_editor_mode])
    ...
    def _on_browse_mode_changed(self):
        if not self.application.browser.hotswap_target and (self._main_modes.selected_mode == 'browse' or self._browser_hotswap_mode.is_entered):
    ...
    def _create_scales(self):
        scales = InstrumentScalesComponent(note_layout=self._note_layout, is_enabled=False, name='Scales', layer=(BackgroundLayer(scale_line1='display_line1', scale_line2='display_line2', priority=consts.DIALOG_PRIORITY), Layer(scale_line1=self.elements.display_line1.subdisplay[:18], scale_line2=self.elements.display_line2.subdisplay[:18], scale_line3=self.elements.display_line3.subdisplay[:9], scale_line4=self.elements.display_line4.subdisplay[9:], top_display_line='select_buttons', bottom_display_line='track_state_buttons', top_buttons='global_param_controls', bottom_buttons='global_param_controls', encoder_controls=self._notification.use_single_line(0, get_slice[18:], align_right), _notification=Layer(presets_toggle_button='shift_button')))
        presets_layer = (BackgroundLayer('track_state_buttons', 'global_param_controls', 'display_line1', 'display_line2', priority=consts.DIALOG_PRIORITY), Layer(top_display_line='display_line3', bottom_display_line='display_line4', top_buttons='select_buttons', _notification=self._notification.use_single_line(0), priority=consts.DIALOG_PRIORITY))
    ...
    def _create_note_settings_component_layer(self):
        return Layer(top_display_line='display_line1', bottom_display_line='display_line2', info_display_line='display_line3', clear_display_line='display_line4', full_velocity_button='accent_button', priority=consts.MOMENTARY_DIALOG_PRIORITY)
    ...
    def _init_stop_clips_action(self):
        self._stop_clips = StopClipComponent(session_ring=self._session_ring, name='Stop_Clip')
    ...
    def _init_quantize_actions(self):
        self._quantize = self._for_non_frozen_tracks(QuantizationComponent(name='Selected_Clip_Quantize', settings_class=self.quantization_settings_class, is_enabled=False, layer=Layer(action_button='quantize_button')))
        self._quantize.settings.layer = (BackgroundLayer('global_param_controls', 'select_buttons', 'track_state_buttons', priority=consts.MOMENTARY_DIALOG_PRIORITY), Layer(swing_amount_encoder='parameter_controls_raw[0]', quantize_to_encoder='parameter_controls_raw[1]', quantize_amount_encoder='parameter_controls_raw[2]', record_quantization_encoder='parameter_controls_raw[7]', record_quantization_toggle_button='track_state_buttons_raw[7]', display_line1='display_line1', display_line2='display_line2', display_line3='display_line3', display_line4='display_line4', priority=consts.MOMENTARY_DIALOG_PRIORITY))

    def _init_fixed_length(self):
        super(Push, self)._init_fixed_length()
        self._fixed_length.settings_component.layer = (BackgroundLayer(self.elements.track_state_buttons.submatrix[:7, :], 'display_line1', 'display_line2', priority=consts.MOMENTARY_DIALOG_PRIORITY), Layer(length_option_buttons='select_buttons', label_display_line='display_line3', option_display_line='display_line4', legato_launch_toggle_button='track_state_buttons_raw[7]', _notification=self._notification.use_single_line(1), priority=consts.MOMENTARY_DIALOG_PRIORITY))
    ...
    def _init_main_modes(self):
        ...
        self.__on_main_mode_button_value.replace_subjects([self.elements.vol_mix_mode_button, self.elements.pan_send_mix_mode_button, self.elements.single_track_mix_mode_button, self.elements.clip_mode_button, self.elements.device_mode_button, self.elements.browse_mode_button])
