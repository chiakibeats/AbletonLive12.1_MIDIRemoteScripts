...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\LV2_LX2_LC2_LD2\LV2MixerController.py
...
class LV2MixerController(FaderfoxMixerController):
    ...
    def reset_status_cache(self):
        self.status_cache = {'mute': [-1 for i in range(0, 12)], 'solo': [-1 for i in range(0, 12)], 'arm': [-1 for i in range(0, 12)], 'current_monitoring_state': [-1 for i in range(0, 12)], 'master': {'solo': -1, 'mute': -1, 'arm': -1}, 'master_note': {'solo': MASTER_SOLO_NOTE, 'mute': MASTER_MUTE_NOTE, 'arm': MASTER_ARM_NOTE}}
        ...
        for idx in range(0, 12):
            ...
            if len(tracks) > idx:
                ...
                if not old_hasattr(track, attr):
                    continue
                elif not track.can_be_armed and attr == 'arm':
                    continue
                else:
