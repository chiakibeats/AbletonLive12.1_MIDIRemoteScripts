...
NOTE_REPEAT_RATES = list(map(_frequency_to_repeat_rate, [_make_triplet(32), 32, _make_triplet(16), 16, _make_triplet(8), 8, _make_triplet(4), 4]))
DEFAULT_INDEX = 5
...
class NoteRepeatComponent(Component):
    ...
    def _get_repeat_rate_index(self):
        ...
        return NOTE_REPEAT_RATES.index(repeat_rate) if repeat_rate in NOTE_REPEAT_RATES else DEFAULT_INDEX
...
class NoteRepeatEnabler(Component):
    ...
    def __init__(self, note_repeat=None, *a, **k):
        ...
        self.note_repeat_component = NoteRepeatComponent(note_repeat=note_repeat, name='Note_Repeat', parent=self, is_enabled=False)
        self.__on_selected_track_changed.subject = self.song.view
