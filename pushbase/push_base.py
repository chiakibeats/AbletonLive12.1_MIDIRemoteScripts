...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\push_base.py
...
class PushBase(ControlSurface):
    ...
    def _init_background(self):
        ...
        self._param_control_background = BackgroundComponent(is_enabled=False, add_nop_listeners=True, layer=Layer(global_param_controls='global_param_controls', priority=consts.BACKGROUND_PRIORITY))
        self._param_control_background.set_enabled(True)

    def _create_background_layer(self):
        return Layer(top_buttons='select_buttons', bottom_buttons='track_state_buttons', scales_button='scale_presets_button', octave_up='octave_up_button', octave_down='octave_down_button', side_buttons='side_buttons', repeat_button='repeat_button', accent_button='accent_button', double_button='double_button', param_touch='global_param_touch_buttons', touch_strip='touch_strip_control', nav_up_button='nav_up_button', nav_down_button='nav_down_button', nav_left_button='nav_left_button', nav_right_button='nav_right_button', new_button='new_button', aftertouch='aftertouch_control', _notification=self._notification.use_single_line(2), priority=consts.BACKGROUND_PRIORITY)
    ...
    def _create_alternating_layout_modes_for_64pads(self, mode='drum', add_touch_strip=False, default_mode=None, alternative_mode=None):
        ...
        loop_selector = LoopSelectorComponent(follow_detail_clip=True, clip_creator=self._clip_creator, name=mode.title() + '_Pad_Loop_Selector', is_enabled=False, layer=Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 0], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:, 0]), default_size=8)
        alternating_layouts = MessengerModesComponent(muted=True, is_enabled=False)
        alternating_layouts.add_mode('64pads', [AddLayerMode(base_component, Layer(matrix='matrix'))] + touch_strip_modes, default_mode=default_mode, alternative_mode=alternative_mode)
        alternating_layouts.add_mode('56pads_loop', [loop_selector, AddLayerMode(base_component, Layer(matrix=self.elements.matrix.submatrix[:, 1:]))] + touch_strip_modes, message=consts.MessageBoxText.ALTERNATE_56_PADS, default_mode=default_mode, alternative_mode=alternative_mode)
    ...
    def _create_slicing_modes(self):
        ...
        self._slicing_loop_modes = create_layouts_for_levels_and_loop(default_mode='sequencer_loop', alternative_mode='sequencer_velocity_levels')
        self._slicing_loop_modes.selected_mode = 'sequencer_loop'
        self._slicing_velocity_levels_modes = create_layouts_for_levels_and_loop(default_mode='sequencer_velocity_levels', alternative_mode='sequencer_loop')
        self._slicing_velocity_levels_modes.selected_mode = 'sequencer_velocity_levels'
        self._slicing_64pads_modes = self._create_alternating_layout_modes_for_64pads(default_mode='64pads', alternative_mode='56pads_loop', **layout_args)
        slicing_modes = MessengerModesComponent(name='Slicing_Modes', is_enabled=False)
        slicing_modes.add_mode('64pads', self._slicing_64pads_modes, message=consts.MessageBoxText.LAYOUT_SLICING_64_PADS)
        self._register_matrix_mode('64pads', modes_component=self._slicing_64pads_modes, parent_path=['matrix_modes', 'note', 'slicing'])
        slicing_modes.add_mode('sequencer_loop', self._slicing_loop_modes, message=consts.MessageBoxText.LAYOUT_SLICING_LOOP)
        self._register_matrix_mode('sequencer_loop', modes_component=self._slicing_64pads_modes, parent_path=['matrix_modes', 'note', 'slicing'])
        slicing_modes.add_mode('sequencer_velocity_levels', self._slicing_velocity_levels_modes, message=consts.MessageBoxText.LAYOUT_SLICING_LEVELS)
        self._register_matrix_mode('sequencer_velocity_levels', modes_component=self._slicing_64pads_modes, parent_path=['matrix_modes', 'note', 'slicing'])
    ...
    def _create_drum_modes(self):
        ...
        create_layouts_for_levels_and_loop = partial(self._create_alternating_layout_modes_for_levels_and_loop, **layout_args)
        self._drum_loop_modes = create_layouts_for_levels_and_loop(default_mode='sequencer_loop', alternative_mode='sequencer_velocity_levels')
        self._drum_loop_modes.selected_mode = 'sequencer_loop'
        self._drum_velocity_levels_modes = create_layouts_for_levels_and_loop(default_mode='sequencer_velocity_levels', alternative_mode='sequencer_loop')
        self._drum_velocity_levels_modes.selected_mode = 'sequencer_velocity_levels'
        self._drum_64pads_modes = self._create_alternating_layout_modes_for_64pads(default_mode='64pads', alternative_mode='56pads_loop', **layout_args)
        drum_modes = MessengerModesComponent(name='Drum_Modes', is_enabled=False)
        drum_modes.add_mode('sequencer_loop', self._drum_loop_modes, message=consts.MessageBoxText.LAYOUT_DRUMS_LOOP)
        self._register_matrix_mode('sequencer_loop', modes_component=self._drum_loop_modes, parent_path=['matrix_modes', 'note', 'drums'])
        drum_modes.add_mode('sequencer_velocity_levels', self._drum_velocity_levels_modes, message=consts.MessageBoxText.LAYOUT_DRUMS_LEVELS)
        self._register_matrix_mode('sequencer_velocity_levels', modes_component=self._drum_loop_modes, parent_path=['matrix_modes', 'note', 'drums'])
        drum_modes.add_mode('64pads', self._drum_64pads_modes, message=consts.MessageBoxText.LAYOUT_DRUMS_64_PADS)
        self._register_matrix_mode('64pads', modes_component=self._drum_loop_modes, parent_path=['matrix_modes', 'note', 'drums'])
    ...
    def _init_matrix_modes(self):
        ...
        self._register_matrix_mode('drums', modes_component=self._drum_modes, parent_path=['matrix_modes', 'note'])
        self._note_modes.add_mode('slicing', [self._slicing_component, self._note_repeat_enabler, self._slicing_modes])
        self._register_matrix_mode('slicing', modes_component=self._slicing_modes, parent_path=['matrix_modes', 'note'])
        self._note_modes.add_mode('looper', self._audio_loop if consts.PROTO_AUDIO_NOTE_MODE else self._matrix_background)
        self._note_modes.add_mode('instrument', [self._note_repeat_enabler, self._instrument, self._scales_enabler])
        self._register_matrix_mode('instrument', modes_component=self._instrument, parent_path=['matrix_modes', 'note'])
        ...
        self._matrix_modes.add_mode('note', self._create_note_mode(), behaviour=self._auto_arm.auto_arm_restore_behaviour())
        self._register_matrix_mode('note', modes_component=self._note_modes, parent_path=['matrix_modes'])
    ...
    def _switch_note_mode_layout(self):
        cyclable_mode = {'instrument': self._instrument, 'drums': self._drum_modes, 'slicing': self._slicing_modes}.get(self._note_modes.selected_mode, None)
    ...
    def _get_current_alternative_layout_mode(self):
        ...
        if note_mode == 'instrument':
            mode = {'play': self._instrument.play_modes, 'sequence': self._instrument.sequence_modes, 'split_melodic_sequencer': self._split_sequencer_mode}.get(self._instrument.selected_mode, None)
        elif note_mode == 'drums':
            mode = {'64pads': self._drum_64pads_modes, 'sequencer_loop': self._drum_loop_modes, 'sequencer_velocity_levels': self._drum_velocity_levels_modes}.get(self._drum_modes.selected_mode, None)
        elif note_mode == 'slicing':
            mode = {'64pads': self._slicing_64pads_modes, 'sequencer_loop': self._slicing_loop_modes, 'sequencer_velocity_levels': self._slicing_velocity_levels_modes}.get(self._slicing_modes.selected_mode, None)
        else:
            mode = None
        return mode
    ...
    def _create_note_mode(self):
        ...
        return [self._view_control, self._note_modes, self._delete_clip, self._select_playing_clip, self._note_layout_switcher]
    ...
    def _create_device_component(self):
        return self.device_component_class(device_decorator_factory=self._device_decorator_factory, device_bank_registry=self._device_bank_registry, banking_info=self._banking_info, name='DeviceComponent', is_enabled=True)
    ...
    def _create_session_recording(self):
        return FixedLengthSessionRecordingComponent(fixed_length_setting=self._fixed_length_setting, clip_creator=self._clip_creator, view_controller=self._view_control, name='Session_Recording')
    ...
    def _init_transport_and_recording(self):
        ...
        session_recording_base_layer = Layer(automation_button='automation_button', new_scene_button=self._with_shift('new_button'), re_enable_automation_button=self._with_shift('automation_button'), delete_automation_button=ComboElement('automation_button', 'delete_button'), foot_switch_button=self.elements.foot_pedal_button.single_press, capture_midi_button=ComboElement('new_button', modifier='record_button'), _uses_foot_pedal='foot_pedal_button')
    ...
    def _init_note_editor_settings_component(self):
        self._note_editor_settings_component = NoteEditorSettingsComponent(note_settings_component_class=self.note_settings_component_class, automation_component_class=self.automation_component_class, grid_resolution=self._grid_resolution, initial_encoder_layer=Layer(initial_encoders='global_param_controls', priority=consts.MOMENTARY_DIALOG_PRIORITY), encoder_layer=Layer(encoders='global_param_controls', priority=consts.MOMENTARY_DIALOG_PRIORITY))
    ...
    def _create_drum_step_sequencer_layer(self):
        return Layer(playhead='playhead_element', full_velocity='full_velocity_element', accent_button='accent_button', button_matrix=self.elements.matrix.submatrix[:8, :4], quantization_buttons='side_buttons', solo_button='global_solo_button', select_button='select_button', delete_button='delete_button', mute_button='global_mute_button', duplicate_button='duplicate_button')

    def _create_split_seq_layer(self):
        return Layer(playhead='playhead_element', full_velocity='full_velocity_element', accent_button='accent_button', button_matrix=self.elements.matrix.submatrix[:8, :4], quantization_buttons='side_buttons', delete_button='delete_button', mute_button='global_mute_button', duplicate_button='duplicate_button')
    ...
    def _init_split_melodic_sequencer(self):
        ...
        self._selected_note_instrument = SelectedNotesInstrumentComponent(note_layout=self._note_layout, note_editor_component=split_seq_note_editor, is_enabled=False, layer=Layer(octave_strip=self._with_shift('touch_strip_control'), octave_up_button='octave_up_button', octave_down_button='octave_down_button', scale_up_button=self._with_shift('octave_up_button'), scale_down_button=self._with_shift('octave_down_button'), matrix=self.elements.matrix.submatrix[:8, 4:8], aftertouch_control='aftertouch_control', delete_button='delete_button', select_button='select_button'))
        ...
        self._split_sequencer_mode.add_mode('default', self._split_melodic_sequencer, default_mode='default', alternative_mode='alternative')
        self._split_sequencer_mode.add_mode('alternative', [self._split_melodic_sequencer, AddLayerMode(self._split_melodic_sequencer, Layer(loop_selector_matrix=self.elements.double_press_matrix.submatrix[:, 4], short_loop_selector_matrix=self.elements.double_press_event_matrix.submatrix[:, 4]))], message=consts.MessageBoxText.LAYOUT_MELODIC_32_PADS_LOOP_SELECTOR, default_mode='default', alternative_mode='alternative')
        self._split_sequencer_mode.selected_mode = 'default'
        self._instrument.add_mode('split_melodic_sequencer', [split_seq_note_editor, self._selected_note_instrument, self._split_sequencer_mode, self._note_editor_settings_component, LayerMode(self._pitch_mod_touch_strip, self._pitch_mod_touch_strip_layer)], message=consts.MessageBoxText.LAYOUT_MELODIC_32_PADS)
        self._register_matrix_mode('split_melodic_sequencer', modes_component=self._split_sequencer_mode, parent_path=['matrix_modes', 'note', 'instrument'])
    ...
    def _init_drum_step_sequencer(self):
        ...
        self._drum_step_sequencer = DrumStepSeqComponent(self._clip_creator, self._skin, name='Drum_Step_Sequencer', grid_resolution=self._grid_resolution, note_editor_component=drum_note_editor, instrument_component=self._drum_component, is_enabled=False)
        self._drum_step_sequencer.layer = self._create_drum_step_sequencer_layer()
        self._audio_loop = LoopSelectorComponent(follow_detail_clip=True, name='Loop_Selector', default_size=8)
    ...
    def _create_slicing_step_sequencer_layer(self):
        return Layer(playhead='playhead_element', full_velocity='full_velocity_element', accent_button='accent_button', button_matrix=self.elements.matrix.submatrix[:8, :4], quantization_buttons='side_buttons', select_button='select_button', duplicate_button='duplicate_button', delete_button='delete_button')
    ...
    def _init_slicing_step_sequencer(self):
        ...
        self._slicing_step_sequencer = StepSeqComponent(self._clip_creator, self._skin, name='Slice_Step_Sequencer', grid_resolution=self._grid_resolution, note_editor_component=slice_note_editor, instrument_component=self._slicing_component, is_enabled=False)
    ...
    def _init_duplicate_actions(self):
        ...
        self._duplicate_loop = self._for_non_frozen_tracks(DuplicateLoopComponent(name='Duplicate_Loop', layer=Layer(action_button='double_button'), is_enabled=False))
    ...
    def _init_value_components(self):
        self._swing_amount = ValueComponent('swing_amount', self.song, display_label='Swing Amount:', display_format='%d%%', model_transform=lambda x: clamp(old_div(x, 200.0), 0.0, 0.5), view_transform=lambda x: x * 200.0, encoder_factor=100.0, encoder_touch_delay=TEMPO_SWING_TOUCH_DELAY)
        self._swing_amount.layer = Layer(encoder='swing_control')
        tempo_param = self.song.master_track.mixer_device.song_tempo
        self._tempo = ValueComponent('tempo', self.song, display_label='Tempo:', display_format='%0.2f BPM', encoder_factor=128.0, encoder_touch_delay=TEMPO_SWING_TOUCH_DELAY, model_transform=lambda x: clamp(x, tempo_param.min, tempo_param.max))
