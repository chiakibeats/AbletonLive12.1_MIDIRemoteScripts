...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\scrollable_list.py
...
class ScrollableList(EventObject, Scrollable):
    ...
    def select_item_index_with_border(self, index, border_size):
        ...
        if self.fixed_offset is not None:
            ...
            if index >= 0 and index < len(self._items) and (not in_range(index, self._offset + border_size, self._offset + self._num_visible_items - border_size)) and (self.selected_item_index < index) and (index - (self._num_visible_items - 2 * border_size) if self.selected_item_index < index else index - (self._num_visible_items - 2 * border_size) if self.selected_item_index < index else index - (self._num_visible_items - 2 * border_size)):
                        else:  # inserted
                            offset = index - border_size
...
class ListComponent(Component):
    ...
    def _trigger_action(self, item):
        if not self.is_enabled() or self._can_be_used_for_action(item):
