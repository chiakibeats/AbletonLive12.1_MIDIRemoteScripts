...
class NoteVelocityDeviationSetting(NoteSetting):
    ...
    def attribute_min_max_to_string(self, min_value, max_value):
        def sign_of(x):
            return '+' if x > 0 else ''
        return sign_of(min_value) + str(int(min_value)) if int(min_value) == int(max_value) else RANGE_STRING_INT % (min_value, max_value)
...
class NoteProbabilitySetting(NoteSetting):
    ...
    def attribute_min_max_to_string(self, min_value, max_value):
        ...
        return '%d %%' % min_value_percent if min_value_percent == max_value_percent else RANGE_STRING_PERCENT % (min_value_percent, max_value_percent)
...
class NoteSettingsComponent(NoteSettingsComponentBase):
    ...
    def set_top_display_line(self, display):
        if not self.is_enabled() or display:
...
def show_clip_view(application):
    try:
        ...
        if not view.is_view_visible('Detail/DeviceChain', False) or not view.is_view_visible('Detail/Clip', False):
...
class NoteEditorSettingsComponent(ModesComponent):
    ...
    def __init__(self, note_settings_component_class=None, automation_component_class=None, grid_resolution=None, initial_encoder_layer=None, encoder_layer=None, *a, **k):
        ...
        self._settings_modes.add_mode('automation', [self.automation, self._mode_selector, partial(self._set_envelope_view_visible, True), partial(show_clip_view, self.application)])
        self._settings_modes.add_mode('note_settings', [self.settings, self._update_note_infos, self._mode_selector, partial(self._set_envelope_view_visible, False), partial(show_clip_view, self.application)])
        ...
        self.add_mode('enabled', [DetailViewRestorerMode(self.application), AddLayerMode(self, encoder_layer), self._settings_modes])
    ...
    def update_view_state_based_on_selected_setting(self, setting):
        if self.selected_mode == 'enabled' and (not self.is_touched):
            pass  # postinserted
        if setting is None:
            ...
            if self._is_step_held():
                if self.selected_setting == 'automation' and (not self.automation.can_automate_parameters):
                    pass  # postinserted
    ...
    def _set_settings_view_enabled(self, should_show_view):
        if self.selected_setting == 'automation':
            return should_show_view and self.automation.can_automate_parameters if True else False
        else:  # inserted
            really_show_view = should_show_view
        if really_show_view:
