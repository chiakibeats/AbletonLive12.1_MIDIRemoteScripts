...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\pushbase\loop_selector_component.py
...
class LoopSelectorComponent(Component, Messenger):
    ...
    def _update_page_selection(self):
        if self.is_enabled():
            if self.is_following and self._has_running_clip():
    ...
    def _update_page_leds_in_matrix(self, matrix):
        pass
        if not self.is_enabled() or matrix:
    ...
    def _try_set_loop(self):
        did_set_loop = False
        if not liveobj_valid(self._sequencer_clip) or not clip_is_new_recording(self._sequencer_clip):
