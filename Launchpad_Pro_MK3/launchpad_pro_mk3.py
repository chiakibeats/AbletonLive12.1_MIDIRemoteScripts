...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_Pro_MK3\launchpad_pro_mk3.py
...
LAYOUT_BYTES_TO_MODE_NAMES_MAP = {ids.SESSION_LAYOUT_BYTES: 'session', ids.CHORD_LAYOUT_BYTES: 'chord', ids.NOTE_LAYOUT_BYTES: 'note'}
...
class Launchpad_Pro_MK3(InstrumentControlMixin, NovationBase):
    ...
    def _setup_faders(self):
        for i, fader_mode in enumerate(FADER_MODES):
            orientation, polarity = (sysex.FADER_HORIZONTAL_ORIENTATION, sysex.FADER_BIPOLAR) if fader_mode == 'pan' else (sysex.FADER_VERTICAL_ORIENTATION, sysex.FADER_UNIPOLAR)
    ...
    def _create_transport(self):
        self._transport = TransportComponent(name='Transport', is_enabled=False, layer=Layer(play_button='play_button', continue_playing_button='play_button_with_shift', metronome_button='solo_button_with_shift', capture_midi_button='record_button_with_shift', tap_tempo_button='sends_button_with_shift'))
        self._transport.tap_tempo_button.color = 'Transport.TapTempo'
        self._transport.set_enabled(True)

    def _create_clip_actions(self):
        self._clip_actions = ClipActionsComponent(name='Clip_Actions', quantization_component=self._quantization, is_enabled=False, layer=Layer(duplicate_button='duplicate_button', quantize_button='quantize_button', double_loop_button='duplicate_button_with_shift'))

    def _create_quantization(self):
        self._quantization = QuantizationComponent(name='Quantization', is_enabled=False, layer=Layer(quantization_toggle_button='quantize_button_with_shift'))
        self._quantization.set_enabled(True)

    def _create_fixed_length(self):
        self._fixed_length = FixedLengthComponent(fixed_length_setting=self._fixed_length_setting, name='Fixed_Length', is_enabled=False, layer=Layer(fixed_length_button='fixed_length_button'))
    ...
    def _create_drum_group(self):
        self._drum_group = DrumGroupComponent(self._clip_actions, name='Drum_Group', is_enabled=False, translation_channel=DRUM_FEEDBACK_CHANNEL, layer=Layer(matrix='drum_pads'))
        self._drum_group.set_enabled(True)

    def _create_device_parameters(self):
        self._device_parameters = SimpleDeviceParameterComponent(name='Device_Parameters', is_enabled=False, device_bank_registry=self._device_bank_registry, layer=Layer(parameter_controls='device_button_faders', static_color_controls='device_button_fader_color_elements', stop_fader_control='stop_fader_element'), static_color_value=Rgb.DARK_BLUE.midi_value)
    ...
    def _create_scale_pad_translator(self):
        self._scale_pad_translator = ConfigurablePlayableComponent(SCALE_FEEDBACK_CHANNEL, name='Scale_Pads', is_enabled=False, layer=Layer(matrix='scale_pads'))
        self._scale_pad_translator.set_enabled(True)

    def _create_mixer_modes(self):
        self._mixer_modes = ModesComponent(name='Mixer_Modes', is_enabled=False, enable_skinning=True, layer=Layer(arm_button='record_arm_button', mute_button='mute_button', solo_button='solo_button', volume_button='volume_button', pan_button='pan_button', sends_button='sends_button', device_button='device_button', stop_button='stop_clip_button'))
        self._mixer.layer = Layer(volume_controls='volume_button_faders', pan_controls='pan_button_faders', send_controls='sends_button_faders')
        ...
        add_track_select_button_mode('stop', control='stop_track_clip_buttons', component=self._session)
        ...
        def add_fader_mode(name, bank, add_layer_mode, static_color=None):
            self._mixer_modes.add_mode(name, (add_layer_mode, AddLayerMode(self._mixer, Layer(track_select_buttons='track_select_buttons')), partial(self._mixer._update_send_control_colors), partial(self._mixer.set_static_color_value, static_color), partial(switch_to_fader_layout, bank)), behaviour=ReenterBehaviour(on_reenter=reselect_track_select_mode))
        add_fader_mode('volume', 0, AddLayerMode(self._mixer, Layer(static_color_controls='volume_button_fader_color_elements')), static_color=Rgb.GREEN.midi_value)
        add_fader_mode('pan', 1, AddLayerMode(self._mixer, Layer(track_color_controls='pan_button_fader_color_elements')))
        add_fader_mode('sends', 2, AddLayerMode(self._mixer, Layer(send_select_buttons='scene_launch_buttons', return_track_color_controls='sends_button_fader_color_elements', stop_fader_control='stop_fader_element')))
        add_fader_mode('device', 3, (AddLayerMode(self._background, Layer(up_button='up_button', down_button='down_button')), AddLayerMode(self._device_navigation, Layer(prev_button='left_button', next_button='right_button')), AddLayerMode(self._device_parameters, Layer(bank_select_buttons='scene_launch_buttons'))))
        self._mixer_modes.selected_mode = 'track_select'
        self._mixer_modes.set_enabled(True)

    def _create_session_modes(self):
        self._session_modes = ModesComponent(name='Session_Modes', is_enabled=False, layer=Layer(overview_button='session_mode_button'))
        self._session_modes.add_mode('launch', AddLayerMode(self._session, Layer(managed_select_button='shift_button', managed_delete_button='clear_button', managed_duplicate_button='duplicate_button', managed_quantize_button='quantize_button', managed_double_button='duplicate_button_with_shift', scene_launch_buttons='scene_launch_buttons')))
        self._session_modes.add_mode('overview', (self._session_overview, AddLayerMode(self._session_navigation, Layer(page_up_button='up_button', page_down_button='down_button', page_left_button='left_button', page_right_button='right_button'))), behaviour=MomentaryBehaviour())
    ...
    def _create_note_modes(self):
        ...
        self._note_modes.add_mode('drum', AddLayerMode(self._drum_group, Layer(scroll_up_button='left_button', scroll_down_button='right_button', scroll_page_up_button='up_button', scroll_page_down_button='down_button', delete_button='clear_button')))

    def _create_main_modes(self):
        self._main_modes = ModesComponent(name='Main_Modes', is_enabled=False)
        suppressed_arrow_button_mode = (AddLayerMode(self._background, Layer(left_button='left_button', right_button='right_button', up_button='up_button', down_button='down_button')),)
