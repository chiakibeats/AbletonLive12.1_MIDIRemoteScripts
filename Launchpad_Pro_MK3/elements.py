...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Launchpad_Pro_MK3\elements.py
...
FADER_MODES = ['volume', 'pan', 'sends', 'device']
...
class Elements(LaunchpadElements):
    ...
    def __init__(self, *a, **k):
        ...
        self.track_select_buttons = ButtonMatrixElement(rows=[self.track_select_buttons_raw], name='Track_Select_Buttons')
        for bank, bank_name in enumerate(FADER_MODES):
            ...
            setattr(self, control_elements_name, ButtonMatrixElement(rows=[[create_slider(index + bank * SESSION_WIDTH, '{}_Button_Fader_{}'.format(capitalized_name, index)) for index in range(SESSION_WIDTH)]], name='{}_Button_Faders'.format(capitalized_name)))
            color_elements_name = '{}_button_fader_color_elements'.format(bank_name)
            setattr(self, color_elements_name, ButtonMatrixElement(rows=[[create_button(index + bank * SESSION_WIDTH, '{}_Button_Fader_Color_Element_{}'.format(capitalized_name, index), channel=BUTTON_FADER_COLOR_CHANNEL) for index in range(SESSION_WIDTH)]], name='{}_Button_Fader_Color_Elements'.format(capitalized_name)))

        def with_shift(button):
            return ComboElement(control=button, modifier=self.shift_button, name='{}_With_Shift'.format(button.name))
        self.track_select_buttons_with_shift = ButtonMatrixElement(rows=[[with_shift(button) for button in self.track_select_buttons_raw]], name='Track_Select_Buttons_With_Shift')
        ...
        self.print_to_clip_element = SysexElement(name='Print_To_Clip_Element', sysex_identifier=print_to_clip_identifier, send_message_generator=lambda v: print_to_clip_identifier + (v, sysex.SYSEX_END_BYTE))
        self.print_to_clip_enabler_element = SysexElement(name='Print_To_Clip_Enabler', send_message_generator=lambda v: sysex.STD_MSG_HEADER + (self.model_id, sysex.PRINT_ENABLE_COMMAND_BYTE, v, sysex.SYSEX_END_BYTE), default_value=0)
        self.fader_setup_element = SysexElement(name='Fader_Setup_Element', send_message_generator=self._fader_setup_message_generator)
        self.stop_fader_element = SysexElement(name='Stop_Fader_Element', send_message_generator=self._stop_fader_message_generator, sysex_identifier=sysex.STD_MSG_HEADER + (self.model_id, sysex.STOP_FADER_COMMAND_BYTE))

    def _stop_fader_message_generator(self, bank):
        return sysex.STD_MSG_HEADER + (self.model_id, sysex.STOP_FADER_COMMAND_BYTE, bank, sysex.SYSEX_END_BYTE)