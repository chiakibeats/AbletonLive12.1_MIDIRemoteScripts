...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\Axiom_AIR_25_49_61\SpecialSessionComponent.py
...
class SpecialSessionComponent(SessionComponent):
    ...
    def _next_track_value(self, value):
        if self.is_enabled() and (value != 0 or not self._next_track_button.is_momentary()):
    ...
    def _prev_track_value(self, value):
        if self.is_enabled() and (value != 0 or not self._prev_track_button.is_momentary()):
            selected_track = self.song().view.selected_track
            all_tracks = self.song().visible_tracks + self.song().return_tracks + (self.song().master_track,)
