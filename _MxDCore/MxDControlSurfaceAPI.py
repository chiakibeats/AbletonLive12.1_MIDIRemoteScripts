...
# Internal filename: ..\..\..\output\Live\win_64_static\Release\python-bundle\MIDI Remote Scripts\_MxDCore\MxDControlSurfaceAPI.py
...
def midi_byte_to_int(byte):
    return int(byte, 0) if isinstance(byte, str) else byte
...
class MxDControlSurfaceAPI(object):
    ...
    class MxDMidiOwner(namedtuple('Owner', 'device_id object_id mxdcore')):
        ...
        def send_reply(self, selector, message):
            ...
            selector_str = {'send_receive': 'send_receive_sysex', 'grab': 'grab_midi', 'release': 'release_midi', 'received': 'received_midi'}.get(selector, str(selector))
            try:
    ...
    def _get_control_or_raise(self, cs, control_or_name, command):
        if not is_control_surface(cs):
            ...
            if isinstance(control_or_name, str):
    ...
    def object_grab_control(self, lom_object, control_or_name):
        cs = lom_object
        control = self._get_control_or_raise(cs, control_or_name, 'grab_control')
        cs.grab_control(control)

    def object_release_control(self, lom_object, control_or_name):
        cs = lom_object
